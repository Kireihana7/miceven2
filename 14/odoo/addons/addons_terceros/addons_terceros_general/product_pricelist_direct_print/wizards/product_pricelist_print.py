...
class ProductPricelistPrint(models.TransientModel):
    ...
    show_variants = fields.Boolean(string="Mostrar Variantes")
    product_tmpl_ids = fields.Many2many(
        comodel_name='product.template',
        string='Plantillas de Productos',
        help='Mantener vacío para todos los productos',
    )
    ...
    show_standard_price = fields.Boolean(string='Mostrar precio de costo')
    show_sale_price = fields.Boolean(string='Mostrar precio de venta')
    ...
    last_ordered_products = fields.Integer(
        help="Si ingresa un X número aquí, entonces, para cada cliente seleccionado,"
             " se obtendrán los últimos X productos pedidos para el informe."
    )
    ...
    def print_report(self):
        if not(self.pricelist_id or self.partner_count
            ...
            raise ValidationError(_(
                'Debe establecer la lista de precios o cualquier cliente '
                'o cualquier opción de mostrar precio.'))
        return self.env.ref(
