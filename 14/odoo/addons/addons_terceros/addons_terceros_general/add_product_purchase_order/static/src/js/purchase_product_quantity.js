odoo.define('add_product_purchase_order.purchase_product_quantity', function (require) {
"use strict";
...
const FSMProductQty = FieldInteger.extend({
    ...
    xmlDependencies: ['/add_product_purchase_order/static/src/xml/purchase_quantity.xml'],
    events: _.extend({}, FieldInteger.prototype.events, {
        'click button[name="purchase_remove_quantity"]': '_removeQuantity',
        'click button[name="purchase_add_quantity"]': '_addQuantity',
        'click span[name="purchase_quantity"]': '_editQuantity',
        'blur span[name="purchase_quantity"]': '_onBlur',
        'keypress span[name="purchase_quantity"]': '_onKeyPress',
        'keydown span[name="purchase_quantity"]': '_onKeyDown',
    }),
    ...
    init: function (parent, name, record, options) {
        ...
        this.mode = 'readonly';
        this.product_id = record.data.id
        //this.product_id = 
    ...
    start: function () {
        ...
        this.$fsmQuantityElement = this.$(`span[name="purchase_quantity"][product_id="${this.product_id}"]`);
    ...
    _removeQuantity: function (e) {
        ...
        if (this._isValid) {
            if (this._isDirty) {
                ...
                this._changeQuantity('purchase_remove_quantity');
            }
        }
    },
    ...
    _addQuantity: async function (e) {
        ...
        if (this._isValid) {
            if (this._isDirty) {
                ...
                this._changeQuantity('purchase_add_quantity');
            }
        }
    },
    ...
    _renderButtons: function () {
        ...
        this.$buttons
            .filter('button[name="purchase_add_quantity"]')
            .toggleClass('btn-light text-muted', this.value === 0);
        this.$buttons
            .filter('button[name="purchase_remove_quantity"]')
