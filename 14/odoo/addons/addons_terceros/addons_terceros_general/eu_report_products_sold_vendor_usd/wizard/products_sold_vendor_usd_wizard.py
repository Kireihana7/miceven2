...
class ProductsSoldVendorUsdWizard(models.TransientModel):
    _name = "products.sold.vendor.usd.wizard"
    ...
    def action_generate_report(self):
        ...
        if self.invoice_user_id:
            param_res_users_id = f'res_users.id = {self.invoice_user_id.id}'

        param_product_id = "product_category.complete_name LIKE '%PRODUCTO TERMINADO%'"
        if self.product_id:
            param_product_id = f'account_move_line.product_id = {self.product_id.id}'            
        ...
        if len(res_res_users) > 0:
            for res_users in res_res_users:
                ...
                if len(invoice_lines) > 0:
                    ...
                    for product_id_loop in product_ids:
                        list_product_data = [elem for elem in invoice_lines if elem['product_id'] == product_id_loop]
                ...
                if len(sold_products_vendor) > 0:
                    sold_products_vendor = sorted(sold_products_vendor, key=lambda d: d['total_quantity'], reverse=True)
                    vendors_data.append({
                        'user_name': user_name,
            ...
            if len(vendors_data) == 0:
                raise ValidationError(_('No se encontraron registros de empleados con ventas realizadas.'))  
            ...
            return self.env.ref('eu_report_products_sold_vendor_usd.action_report_products_sold_vendor_usd').report_action(self, data=data)                  
        else:
            raise ValidationError(_('No se encontraron registros según los datos ingresados.'))   