...
class WizardBookPurchaseXls(models.TransientModel):
    ...
    def _get_report_values(self, data=None):
        form_data = data['form']
        date_start = datetime.datetime.strptime(str(form_data['date_from']), '%Y-%m-%d').date()
        date_end = datetime.datetime.strptime(str(form_data['date_to']), '%Y-%m-%d').date()
    ...
    def print_xls_report(self):
        ...
        for u in facs.sorted(lambda m: m.invoice_date):
            ...
            if u.state !='annulled':
                ...
                if u.move_type == 'in_invoice':
                    total = total + sum(u.mapped('amount_total'))
                ...
                if len(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'reduced')))):
                    ...
                    if u.move_type == 'in_invoice': 
                        R333 = R333 + sum(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal'))
                    if u.move_type == 'in_refund':
                        R333 = R333 -sum(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal'))
                    #Monto del Impuesto
                    if u.move_type == 'in_invoice':
                        R343 = R343 + (sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal')))
                    if u.move_type == 'in_refund':
                        R343 = R343 - (sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal')))
                    ...
                    if u.wh_id:
                        ...
                        if u.move_type == 'in_refund':
                            retencion_r = retencion_r-((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal')))*retention)
                        if u.move_type == 'in_invoice':
                            retencion_r = retencion_r+((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal')))*retention)
                        #Iva Percibido
                        if u.move_type == 'in_refund':
                            iva_percibido_r = iva_percibido_r-(sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal'))-((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal')))*retention))
                        if u.move_type == 'in_invoice':
                            iva_percibido_r = iva_percibido_r+(sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal'))-((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'reduced'))).mapped('price_subtotal')))*retention))
                ...
                if len(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'additional')))):
                    #Base imponible del Impuesto
                    if u.move_type == 'in_invoice':
                        R332 = R332 +sum(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal'))
                    if u.move_type == 'in_refund':
                        R332 = R332 -sum(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal'))
                    #Monto del Impuesto
                    if u.move_type == 'in_invoice':
                        R342 = R342 + (sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal')))
                    if u.move_type == 'in_refund':
                        R342 = R342 - (sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal')))
                    #Iva Percibido
                    #Iva Percibido - IVA RETENIDO POR EL COMPRADOR
                    if u.wh_id:
                        #IVA RETENIDO POR EL COMPRADOR
                        if u.move_type == 'in_refund':
                            retencion_ga = retencion_ga-((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal')))*retention)
                        if u.move_type == 'in_invoice':
                            retencion_ga = retencion_ga+((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal')))*retention)
                        #Iva Percibido  -->
                        if u.move_type == 'in_refund':
                            iva_percibido_ga = iva_percibido_ga-(sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal'))-((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal')))*retention))
                        if u.move_type == 'in_invoice':
                            iva_percibido_ga = iva_percibido_ga+(sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal'))-((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'additional'))).mapped('price_subtotal')))*retention))
                ...
                if len(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'general')))):
                    #Base imponible del Impuesto
                    if u.move_type == 'in_invoice':
                        R33 = R33 +sum(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'general'))).mapped('price_subtotal'))
                    if u.move_type == 'in_refund':
                        R33 = R33 -sum(u.invoice_line_ids.filtered(lambda line: len(line.mapped('tax_ids').filtered(lambda tax: tax.aliquot_type == 'general'))).mapped('price_subtotal'))
                    #Monto del Impuesto
                    if u.move_type == 'in_invoice':
                        R34 = R34 + (sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'general'))).mapped('price_subtotal')))
                    if u.move_type == 'in_refund':
                        R34 = R34 - (sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'general'))).mapped('price_subtotal')))
                    #Iva Percibido - IVA RETENIDO POR EL COMPRADOR
                    if u.wh_id:
                        #IVA RETENIDO POR EL COMPRADOR
                        if u.move_type == 'in_refund':
                            retencion_g = retencion_g-((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'general'))).mapped('price_subtotal')))*retention)
                        if u.move_type == 'in_invoice':
                            retencion_g = retencion_g+((sum(u.line_ids.filtered(lambda line: len(line.mapped('tax_line_id').filtered(lambda tax: tax.aliquot_type == 'general'))).mapped('price_subtotal')))*retention)
                        #Iva Percibido
                        if u.move_type == 'in_refund':
                ...
                #Retenciones
                if u.move_type == 'in_invoice':
                    retencion = retencion + u.wh_id.total_tax_ret
                if u.move_type == 'in_refund':
                    retencion = retencion - u.wh_id.total_tax_ret
                #TOTAL de IMPUESTOS (Reducido + General + Adicional)
                if u.move_type == 'in_invoice':
                    tax = tax + u.amount_tax
                if u.move_type == 'in_refund':
                    tax = tax - u.amount_tax
                #IVA PERCIBIDO
                if u.move_type == 'in_invoice': 
                    iva_percibido = iva_percibido+((u.amount_tax)-(u.wh_id.total_tax_ret))
                if u.move_type == 'in_refund':
                    iva_percibido = iva_percibido-((u.amount_tax)-(u.wh_id.total_tax_ret))
                ...
                monto_impuesto_untaxed = sum(u.invoice_line_ids.filtered(lambda whl: not whl.tax_ids).mapped('price_subtotal'))
                ...
                if monto_impuesto == 0 or monto_impuesto_untaxed > 0 or monto_impuesto_untaxed_exento:
                    ...
                    acum_exento = acum_exento + (exento)
                else:
                ...
                if not u.deductible:
                    if u.move_type == 'in_invoice':
                        tax_deductible = tax_deductible + u.amount_tax
                    if u.move_type == 'in_refund':
                        tax_deductible = tax_deductible - u.amount_tax
                    if u.move_type == 'in_invoice':
                        untaxed_deductible = untaxed_deductible + sum(u.invoice_line_ids.filtered(lambda whl: whl.tax_ids).mapped('price_subtotal'))
                    if u.move_type == 'in_refund':
                        untaxed_deductible = untaxed_deductible - sum(u.invoice_line_ids.filtered(lambda whl: whl.tax_ids).mapped('price_subtotal'))
                    if sum(u.invoice_line_ids.mapped('tax_ids.amount')) > 0:
                        worksheet.write(rows,17, (sum(u.mapped('amount_total')) - (acum_exento + u.amount_tax))*-1 if u.move_type == 'in_refund' else sum(u.mapped('amount_total')) - (acum_exento + u.amount_tax), currency_format)
                    else:
                ...
                if u.deductible:
                    if u.move_type == 'in_invoice':
                        tax_not_deductible = tax_not_deductible + u.amount_tax
                    if u.move_type == 'in_refund':
                        tax_not_deductible = tax_not_deductible - u.amount_tax
                    if u.move_type == 'in_invoice':
                        untaxed_not_deductible = untaxed_not_deductible + sum(u.invoice_line_ids.filtered(lambda whl: whl.tax_ids).mapped('price_subtotal'))
                    if u.move_type == 'in_refund':
                        untaxed_not_deductible = untaxed_not_deductible - sum(u.invoice_line_ids.filtered(lambda whl: whl.tax_ids).mapped('price_subtotal'))
        ...
        for ret in dict_fact['rete'].sorted(lambda r: (r.retention_id.date, r.retention_id.id)):
            ...
            acumret = acumret + ret.ret_amount
        ...
        #DEDUCIBLE INICIO
        worksheet.write(rows+1,17, R33, currency_format)
        ...
        R342 =R342-R322
        R343 =R343-R323
        R35 =R30+R332+R333+R33+R312+R313
