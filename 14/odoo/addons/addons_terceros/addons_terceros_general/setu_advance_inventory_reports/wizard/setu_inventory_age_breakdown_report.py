...
class SetuInventoryAgeBreakdownReport(models.TransientModel):
    ...
    def set_report_title(self, workbook, worksheet):
        ...
        worksheet.merge_range(0, 0, 1, 17, "Reporte de quiebre de inventario por antiguedad", wb_format)
    ...
    def get_column_header(self, counter):
        ...
        if counter == 7:
            header = "Más viejos que " + str(((counter - 1) * bbd) + 1)
            return header

        header = str(((counter - 1) * bbd) + 1) + " a " + str(counter * bbd)
        return header
    ...
    def write_report_data_header(self, workbook, worksheet, row):
        ...
        worksheet.write(row, 0, 'Producto', normal_left_format)
        worksheet.write(row, 1, 'Categoría', normal_left_format)
        worksheet.write(row, 2, 'Cant. total', odd_normal_right_format)
        worksheet.write(row, 3, 'Valor', even_normal_right_format)

        self.set_breakdown_header(workbook, worksheet, row-1, 4, self.get_column_header(1), odd_normal_center_format)
        worksheet.write(row, 4, "Cant." ,odd_normal_right_format)
        worksheet.write(row, 5, "Valor", odd_normal_right_format)

        self.set_breakdown_header(workbook, worksheet, row - 1, 6, self.get_column_header(2), even_normal_center_format)
        worksheet.write(row, 6, "Cant.", even_normal_right_format)
        worksheet.write(row, 7, "Valor", even_normal_right_format)

        self.set_breakdown_header(workbook, worksheet, row - 1, 8, self.get_column_header(3), odd_normal_center_format)
        worksheet.write(row, 8, "Cant.", odd_normal_right_format)
        worksheet.write(row, 9, "Valor", odd_normal_right_format)

        self.set_breakdown_header(workbook, worksheet, row - 1, 10, self.get_column_header(4), even_normal_center_format)
        worksheet.write(row, 10, "Cant.", odd_normal_right_format)
        worksheet.write(row, 11, "Valor", odd_normal_right_format)

        self.set_breakdown_header(workbook, worksheet, row - 1, 12, self.get_column_header(5), odd_normal_center_format)
        worksheet.write(row, 12, "Cant.", odd_normal_right_format)
        worksheet.write(row, 13, "Valor", odd_normal_right_format)

        self.set_breakdown_header(workbook, worksheet, row - 1, 14, self.get_column_header(6), even_normal_center_format)
        worksheet.write(row, 14, "Cant.", odd_normal_right_format)
        worksheet.write(row, 15, "Valor", odd_normal_right_format)

        self.set_breakdown_header(workbook, worksheet, row - 1, 16, self.get_column_header(7), odd_normal_center_format)
        worksheet.write(row, 16, "Cant.", odd_normal_right_format)
        worksheet.write(row, 17, "Valor", odd_normal_right_format)
