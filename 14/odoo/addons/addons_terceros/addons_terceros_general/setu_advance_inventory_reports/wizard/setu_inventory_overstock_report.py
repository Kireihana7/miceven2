...
class SetuInventoryOverstockReport(models.TransientModel):
    ...
    def set_report_title(self, workbook, worksheet):
        ...
        worksheet.merge_range(0, 0, 1, 20, "Reporte excedente de inventario", wb_format)
        ...
        report_string = "Analisis de Inventario de los siguientes"
        worksheet.merge_range(2, 0, 2, 1, report_string, wb_format_left)
        worksheet.write(2, 2, str(self.advance_stock_days) + " dias", wb_format_center)

        worksheet.merge_range(3, 0, 3, 1, "Historial de venta desde", wb_format_left)
        worksheet.merge_range(4, 0, 4, 1, "Hasta", wb_format_left)
    ...
    def write_report_data_header(self, workbook, worksheet, row):
        ...
        worksheet.write(row, 0, 'Producto', normal_center_format)
        worksheet.write(row, 1, 'Categoría', normal_center_format)
        worksheet.write(row, 2, 'Ventas', normal_center_format)
        worksheet.write(row, 3, 'ADS', normal_center_format)
        worksheet.write(row, 4, 'Cant. Actual', normal_center_format)
        worksheet.write(row, 5, 'Salientes', normal_center_format)
        worksheet.write(row, 6, 'Entrantes', normal_center_format)
        worksheet.write(row, 7, 'Inv. Virtual', normal_center_format)
        worksheet.write(row, 8, 'Cant. demandada', normal_center_format)
        worksheet.write(row, 9, 'Dias Cubiertos', normal_center_format)
        worksheet.write(row, 10, 'Cant. Excedente', normal_center_format)
        worksheet.write(row, 11, 'Valor Excedente', normal_center_format)
        worksheet.write(row, 12, 'Ratio de reposición', normal_center_format)
        worksheet.write(row, 13, 'Clasificación FSN', normal_center_format)
        worksheet.write(row, 14, 'Cant. excedente (%)', normal_center_format)
        worksheet.write(row, 15, 'Valor excedente (%)', normal_center_format)
        worksheet.write(row, 16, 'Fecha ult. PO', normal_center_format)
        worksheet.write(row, 17, 'Cant. ult. PO', normal_center_format)
        worksheet.write(row, 18, 'Precio ult. PO', normal_center_format)
        worksheet.write(row, 19, 'Moneda', normal_center_format)
        worksheet.write(row, 20, 'Vendedor', normal_center_format)
        return worksheet
