...
class SetuStockMovementReport(models.TransientModel):
    ...
    def set_report_title(self, workbook, worksheet):
        ...
        worksheet.merge_range(0, 0, 1, 16, "Reporte de Mov. de Inventario", wb_format)
        start_date, end_date = self.get_report_date_range()
        ...
        if start_date == '1900-01-01':
            report_string = "Movimientos hasta"
    ...
    def write_report_data_header(self, workbook, worksheet, row):
        ...
        worksheet.write(row, 0, 'Compañia', odd_normal_center_format)
        # product name = 2 column
        worksheet.write(row, 1, 'Producto', odd_normal_center_format)
        # category name = 3 column
        worksheet.write(row, 2, 'Categoría', odd_normal_center_format)
        # opening stock = 4 column
        worksheet.write(row, 3, 'Cant. Inicial', odd_normal_center_format)
        # sales = 5 column
        worksheet.write(row, 4, 'Ventas', odd_normal_center_format)
        # sales_return = 6 column
        worksheet.write(row, 5, 'Ventas dev.', odd_normal_center_format)
        # purchase = 7 column
        worksheet.write(row, 6, 'Compras', odd_normal_center_format)
        # purchase_return = 8 column
        worksheet.write(row, 7, 'Compras dev.', odd_normal_center_format)
        # internal_in = 9 column
        worksheet.write(row, 8, 'Transf. entrada I.', odd_normal_center_format)
        # internal_out = 10 column
        worksheet.write(row, 9, 'Transf. salida I.', odd_normal_center_format)
        # adjustment_in = 11 column
        worksheet.write(row, 10, 'Entrada Ajust.', odd_normal_center_format)
        # adjustment_out = 12 column
        worksheet.write(row, 11, 'Salida Ajust.', odd_normal_center_format)
        # production_in = 13 column
        worksheet.write(row, 12, 'Entrada Producción', odd_normal_center_format)
        # production_out = 14 column
        worksheet.write(row, 13, 'Salida Producción', odd_normal_center_format)
        # transit_in = 15 column
        worksheet.write(row, 14, 'Entrada Transito', odd_normal_center_format)
        # transit_out = 16 column
        worksheet.write(row, 15, 'Salida Transito', odd_normal_center_format)
        # closing = 16 column
        worksheet.write(row, 16, 'Cierre', odd_normal_center_format)
