...
class AttendanceReport(models.TransientModel):
    _name = 'account.report.overdue.cxp'
    ...
    agrupar_clientes = fields.Boolean(
        string='Agrupar por Proveedores',
    )
    ...
    def print_report(self):
        ...
        if self.res_partner_id:
            domain.append(('partner_id','=',self.res_partner_id.id))
        ...
        if len(codes)>0:
            domain.append(('journal_id', 'in', codes))
        ...
        #raise UserError(cliente_dict)
        data = {
            'form': res,
        }
        return self.env.ref('eu_account_overdue.account_overdue_print_cxp').report_action([],data=data)