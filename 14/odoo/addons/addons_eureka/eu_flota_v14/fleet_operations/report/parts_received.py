...
class ReceivedPartsXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, res, stock_picking_ids):
        ...
        for picking in stock_picking_ids:
            ...
            counter = 1
            worksheet.write(row, 0, 'DATE RECEIVED:' if picking.date_done else picking.date_deadline, tot)
            worksheet.write(row, 2, picking.date_done if picking.date_done else picking.date_deadline)
            row += 1
            for line in picking.move_lines:
                if picking.state == 'done' and purchase_id:
                    ...
                    worksheet.write(row, 11,
                                    picking.product_id.location_id.name)
                    row += 2
                    counter += 1
        row += 5
        fp = io.BytesIO()
