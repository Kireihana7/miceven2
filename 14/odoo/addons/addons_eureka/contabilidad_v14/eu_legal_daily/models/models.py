...
class AccountReportLegal(models.TransientModel):
    ...
    in_bs = fields.Boolean(string="En Bs?",default=True)
                        ...
                        if self.date_from:
                            ...
                            if self.show_initial_balance == True:
                                if child_line.date < self.date_from:
                                    debit_initial   += child_line.debit_usd
                                    credit_initial  += child_line.credit_usd
                                    balance_initial += (child_line.debit_usd - child_line.credit_usd)

                            if child_line.date >= self.date_from and child_line.date <= self.date_to:
                                
                                if self.target_move == 'all':
                                    debit_final += child_line.debit_usd
                                    credit_final += child_line.credit_usd
                        ...
                        #SI NO HAY FECHAS
                            ...
                            if self.target_move == 'posted':
                               if child_line.parent_state == 'posted':
                                   debit_final += child_line.debit_usd
                                   credit_final += child_line.credit_usd
                                   balance_final += (child_line.debit_usd - child_line.credit_usd)
