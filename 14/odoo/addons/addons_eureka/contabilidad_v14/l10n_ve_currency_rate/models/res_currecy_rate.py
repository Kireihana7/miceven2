...
class ResCurrencyRateServer(models.Model):
    ...
    def central_bank(self):
        ...
        if status_code == 200:
            ...
            # Yuan
            yuan = html.find('div', {'id': 'yuan'})
            yuan = str(yuan.find('strong')).split()
            yuan = str.replace(yuan[1], '.', '')
            yuan = float(str.replace(yuan, ',', '.'))
            if self.name.name == 'USD':
                bcv = 1/dolar
            elif self.name.name == 'EUR':
                bcv = euro
            elif self.name.name == 'CNY':
                bcv = yuan
    ...
    def set_rate(self):
        ...
        if len(rate) == 0: 
            self.env['res.currency.rate'].create({
                ...
                'name': datetime.now(),
                'rate_inv': 1 / round(currency, 10),
                'rate': round(currency, 4),
                'company_id':self.company_id.id,
            })
        else :
            rate.rate = round(currency, 4)
            rate.rate_inv = 1 / round(currency, 10)
        #if self.name.id == 2:
...
class ResCurrencyRate(models.Model):
    ...
    def get_systray_dict(self, date):
        ...
        if rate:
            exchange_rate =  1 / rate.rate_inv
            return {'date': _('Date : ') + rate.name.strftime("%d/%m/%Y"), 'rate': "Bs/USD: "+ str("{:,.4f}".format(exchange_rate))  }
        else:
