        ...
        for rec in self:
            ...
            if tiene_servicio:
                if rec.move_type == 'in_invoice':
                    ...
                    base_imponible_abs = rec.amount_untaxed if rec.currency_id != self.env.company.currency_id else rec.amount_untaxed_ref
                    if float(base_imponible_abs) < float(wh_table_retention_line.apply_up_to):
                    ...
                    if ret_amount > 0:
                        lines.append([0, False, {
                            ...
                            'base_tax': rec.amount_untaxed / rec.manual_currency_exchange_rate if rec.currency_id != self.env.company.currency_id else rec.amount_untaxed,
                            'base_tax_bs': rec.amount_untaxed * rec.manual_currency_exchange_rate if rec.currency_id == self.env.company.currency_id else rec.amount_untaxed,
