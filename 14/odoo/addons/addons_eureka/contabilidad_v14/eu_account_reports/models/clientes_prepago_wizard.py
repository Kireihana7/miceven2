...
class AccountReceivableFpGeneralWizard(models.TransientModel):
    ...
    def action_generate_report(self):
        ...
        mis_ids = ','.join([str(i) for i in self.partner_ids.ids])
        if self.partner_ids:
            param_partner_ids = f'partner_id in ({mis_ids})'
        ...
        if len(sale_orders) == 0:
            raise ValidationError(
                _('No se encontraron ventas según los datos ingresados.'))
        else:
            for sale_order in sale_orders:
                ...
                if len(res_sale_order_lines) > 0:
                    for sale_line in res_sale_order_lines:
                        ...
                        if product_uom is not None:
                            ...
                            if len(res_uom_uom_po_line) > 0:
                                ...
                                factor_inv = self.env['uom.uom'].search(
                                    [('id', '=', str(uom_id))]).factor_inv
                                if factor_inv:
                        ...
                        # Líneas de la venta:
                        cantidad_x_despachar = sale_line['product_uom_qty'] - \
                    ...
                    total_qty_despachar_x_fardos_so = qty_x_fardos_negociadas_so - \
                        qty_x_fardos_despachadas_so
        ...
        total_qty_x_kg_negociadas_despachadas = total_qty_x_kg_cargas_negociadas - \
            total_qty_x_kg_cargas_despachadas

        if len(sales) == 0:
            raise ValidationError(
                _('No se encontraron ventas con movimientos de inventario.'))
        ...
        return self.env.ref('eu_account_reports.action_report_clientes_prepago').report_action(self, data=data)
