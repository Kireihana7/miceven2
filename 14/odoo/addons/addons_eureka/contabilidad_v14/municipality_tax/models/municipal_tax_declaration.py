...
class MunicipalTaxDeclaration(models.Model):
    ...
    entry_payment = fields.Many2one('account.move',string="Asiento del Pago IAE",readonly=True)
    total_payment = fields.Many2one('account.payment',string="Pago Relacionado",readonly=True)
    iae_payment = fields.Many2one('account.retention.pay',string="Pago del IAE",readonly=True)
    ...
    def act_getfile(self,declaration):
        ...
        for wh in declaration.tax_municipal_line:
                ...
                content += '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n'%(
                    ...
                    str(round(wh.base_imponible if wh.municipal_tax_id.invoice_id == self.env.company.currency_id else wh.base_imponible * wh.municipal_tax_id.manual_currency_exchange_rate,2)).replace(".",","),
                    str(round(wh.aliquot,2)).replace(".",",")+'0'+'%',
                    str(round(wh.amount_total_ret * wh.municipal_tax_id.manual_currency_exchange_rate,2)).replace(".",","),
                    )
