...
class IvaWizard(models.TransientModel):
    ...
    in_bs =fields.Boolean(default=True, string="En Bolivares")
    def report_iae(self):
        final = []
        date_clause = ""
        query_params = []
        ...
        sql_iae = ("""
           ...
           SELECT municipality_tax.name as comprobante ,
            ...
            account_move.manual_currency_exchange_rate as tasa,

            account_move.currency_id as moneda,
            ...
            if self.in_bs:
                for row in query_result:
                    if row['moneda'] == self.env.company.currency_id.id:
                        ...
                        base_imponible = row['base_imponible'] * row['tasa'] 
                        monto_retenido= row['monto_retenido'] * row['tasa']
                        alicuota = row['alicuota'] * row['tasa']
                    else: 
                        monto_total= row['monto_total'] / row['tasa']
                        base_imponible = row['base_imponible'] / row['tasa'] 
                        monto_retenido = row['monto_retenido'] / row['tasa']
                ...
                for row in query_result:
                    if row['moneda'] == self.env.company.currency_id.id:
                        ...
                        monto_total= row['monto_total'] / row['tasa']
                        base_imponible = row['base_imponible'] / row['tasa'] 
                    monto_retenido = row['monto_retenido'] 
