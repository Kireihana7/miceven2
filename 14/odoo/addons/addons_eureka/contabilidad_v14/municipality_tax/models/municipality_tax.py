...
class PeriodMonth(models.Model):
    _name = 'period.month'
    _description="Periodo Meses IAE"
...
class MunicipalityTaxLine(models.Model):
    ...
    def _aliq_mun(self):
        for x in self:
            ...
            if x.municipality_tax_id.move_type in ('out_invoice','out_refund'):
                if x.concept_partner2:
                    x.concept_id = x.concept_partner2
            if x.municipality_tax_id.move_type in ('in_invoice','in_refund'):
                x.move_type = 'purchase'
...
class MUnicipalityTax(models.Model):
    ...
    def action_post(self):
        ...
        if not self.asiento_post:
            ...
            asiento_rete = moves.line_ids\
            .filtered(lambda line: line.account_id.user_type_id.type in ('receivable', 'payable'))
            factura =  self.invoice_id.line_ids.filtered(lambda line: line.account_id.user_type_id.type in ('receivable', 'payable') and not line.reconciled)
            if self.state =='posted':
                (asiento_rete + factura).reconcile()
    ...
    def registro_movimiento_retencion(self,consecutivo_asiento):
        ...
        value = {
            ...
            'currency_id':self.invoice_id.company_id.currency_id.parent_id.id,
        }
    ...
    def registro_movimiento_linea_retencion(self,id_movv,consecutivo_asiento):
        name = consecutivo_asiento
        valores = self.amount 
        valores_ref = self.amount * self.invoice_id.manual_currency_exchange_rate
        cero = 0.0
        value = []
        if self.move_type in ('out_invoice','out_refund','out_receipt'):
