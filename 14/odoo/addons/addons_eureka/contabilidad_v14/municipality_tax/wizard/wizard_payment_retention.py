...
class AccountPayRetention(models.Model):
    ...
    def pay_retention_tax_municipal(self):
        ...
        for pay in self:
            amount_currency = 0
            for muni in pay.tax_municipal.tax_municipal_line:
                amount_currency += muni.amount_total_ret * muni.municipal_tax_id.manual_currency_exchange_rate
            payment_dict = {
            ...
            payment.write({'iae_entry':move.id})
            move.post()
            ...
            pay.tax_municipal.write({'state':'done','entry_payment':move.id,'total_payment':payment.id,'iae_payment':self.id})
