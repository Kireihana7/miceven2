        ...
        for rec in self.env['account.move'].sudo().search(['|', ('period_id', '=', False), ('fiscal_year', '=', False)]):
            if rec.date:
                period = self.env['sh.account.period'].sudo().search(
                    [('date_start', '<=', rec.date), ('date_end', '>=', rec.date),('company_id','=',self.env.company.id)], limit=1)
                if period:
...
class Move(models.Model):
    ...
    def create(self, vals_list):
        ...
        #     if 'date' in vals:
        #         period = self.env['sh.account.period'].sudo().search(
        #             [('date_start', '<=', vals['date']), ('date_end', '>=', vals['date']),('company_id','=',self.env.company.id)], limit=1 )
        ...
        for rec in self:
            if rec.date and rec.company_id:
                ...
                if rec.company_id.sh_restrict_for_close_period and period.state == 'done':
                    raise UserError(_(
                        "You can not Select Date from Closed Fiscal Period / Closed Fiscal Year. Nombre: %s, ID: %s") % (rec.name,rec.id))
        return res

    def write(self, vals):
        rslt = super(Move, self).write(vals)
        if self and not self._context.get('force_write'):
            for rec in self:
                if rec.company_id.sh_restrict_for_close_period and rec.period_id.state == 'done' and not rec.payment_id:
                    raise UserError(_(
                        "You can not Select Date from Closed Fiscal Period / Closed Fiscal Year. Nombre: %s, ID: %s") % (rec.name,rec.id))
        return rslt
...
class ShFiscalYear(models.Model):
    ...
    move_id = fields.Many2one('account.move', string="End of Year Entries Journal",
                              readonly=True, states={'draft': [('readonly', False)]})
    company_id = fields.Many2one('res.company',string="Compañía", required=True, default=lambda self: self.env.company,invisible=True)
    ...
    def re_open_fiscal_year(self):
        for rec in self:
            if rec.env.company.sh_enable_approval:
