...
class report_account_consolidated_journal(models.AbstractModel):
    _name = "account.consolidated.journal.usd"
    ...
    def _get_sum(self, results, lambda_filter):
        sum_debit = self.format_value_usd(sum([r['debit'] for r in results if lambda_filter(r)]))
        sum_credit = self.format_value_usd(sum([r['credit'] for r in results if lambda_filter(r)]))
        sum_balance = self.format_value_usd(sum([r['balance'] for r in results if lambda_filter(r)]))
    ...
    def _get_line_total_per_month(self, options, current_company, results):
        ...
        if dates:
            ...
            for date in sorted(dates):
                ...
                sum_debit = self.format_value_usd(sum([r['debit'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company]))
                sum_credit = self.format_value_usd(sum([r['credit'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company]))
                sum_balance = self.format_value_usd(sum([r['balance'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company]))
                vals = {
    ...
    def _get_lines(self, options, line_id=None):
        ...
        select = """
            SELECT to_char("account_move_line".date, 'MM') as month,
                   ...
                   COALESCE(SUM("account_move_line".debit_usd), 0) as debit,
                   COALESCE(SUM("account_move_line".credit_usd), 0) as credit,
                   j.id as journal_id,
        ...
        for values in results:
            ...
            if self._need_to_unfold('account_%s_%s' % (values['account_id'], values['journal_id']), options):
                vals = {
                    ...
                    'columns': [{'name': n} for n in [self.format_value_usd(values['debit']), self.format_value_usd(values['credit']), self.format_value_usd(values['balance'])]],
                }
                lines.append(vals)
