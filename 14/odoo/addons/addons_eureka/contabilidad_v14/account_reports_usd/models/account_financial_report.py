...
class ReportAccountFinancialReport(models.Model):
    _name = "account.financial.html.report.usd"
    ...
    line_ids = fields.One2many('account.financial.html.report.usd.line', 'financial_report_id', string='Lines')
    ...
    def _format_cell_value(self, financial_line, amount, currency=False, blank_if_zero=False):
        ...
        :param financial_line:  An account.financial.html.report.usd.line record.
        :param amount:          A number.
        :param currency:        An optional res.currency record.
        :param blank_if_zero:   An optional flag forcing the string to be empty if amount is zero.
        :return:
    ...
    def _build_lines_hierarchy(self, options_list, financial_lines, solver, groupby_keys):
        ...
        :param options_list:        The report options list, first one being the current dates range, others being the
                                    comparisons.
        :param financial_lines:     An account.financial.html.report.usd.line recordset.
        :param solver:              The FormulaSolver instance used to compute the formulas.
    ...
    def _get_lines(self, options, line_id=None):
        ...
        financial_line = self.env['account.financial.html.report.usd.line'].browse(line_id)
    ...
    def _get_table(self, options):
        ...
        options_list = self._get_options_periods_list(options)
        formula_solver = FormulaSolver(options_list, self)
        financial_lines = self.env['account.financial.html.report.usd.line'].search([('id', 'child_of', self.line_ids.ids)])
    ...
    def _get_financial_line_report_line(self, options, financial_line, solver, groupby_keys):
        ''' Create the report line for an account.financial.html.report.usd.line record.
        :param options:             The report options.
        :param financial_line:      An account.financial.html.report.usd.line record.
    ...
    def copy(self, default=None):
        ...
        :return: The copied account.financial.html.report.usd record.
        '''
        self.ensure_one()
        if default is None:
            default = {}
        ...
        for report in self:
            if not report.generated_menu_id:
                action_vals = {
                    ...
                    'tag': 'account_report',
                    'context': {
                        'model': 'account.financial.html.report.usd',
                        'id': report.id,
                    },
                }
                action_xmlid = "%s.%s" % (module, 'account_financial_html_report_usd_action_' + str(report.id))
                ...
                menu_xmlid = "%s.%s" % (module, 'account_financial_html_report_usd_menu_' + str(report.id))
    ...
    def _get_copied_name(self):
        '''Return a copied name of the account.financial.html.report.usd record by adding the suffix (copy) at the end
        until the name is unique.

        :return: an unique name for the copied account.financial.html.report.usd
        '''
        self.ensure_one()
...
class AccountFinancialReportLine(models.Model):
    _name = "account.financial.html.report.usd.line"
    ...
    financial_report_id = fields.Many2one('account.financial.html.report.usd', 'Financial Report')
    parent_id = fields.Many2one('account.financial.html.report.usd.line', string='Parent', ondelete='cascade')
    children_ids = fields.One2many('account.financial.html.report.usd.line', 'parent_id', string='Children')
    ...
    def _check_line_consistency(self):
        ...
        for rec in self:
            ...
            if rec.groupby:
                ...
                if not groupby_field or not self.env['account.financial.html.report.usd']._is_allowed_groupby_field(groupby_field):
    ...
    def _get_financial_report(self):
        ...
        :return: An account.financial.html.report.usd record.
        '''
        self.ensure_one()
        line = self
    ...
    def _get_copied_code(self):
        ...
        :return: an unique code for the copied account.financial.html.report.usd.line
        '''
        self.ensure_one()
