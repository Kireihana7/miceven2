...
class ReportAccountAgedPartner(models.AbstractModel):
    _name = "account.aged.partner.usd"
    _description = "Aged Partner Balances"
    _inherit = 'account.accounting.report.usd'
    ...
    period1 = fields.Monetary(string='1 -  7')
    period2 = fields.Monetary(string='8 -  15')
    period3 = fields.Monetary(string='16 - 21')
    period4 = fields.Monetary(string='22 - 28')
    ...
    def _field_column_usd(self, field_name, sortable=False, name=None):
        ...
        if self._fields[field_name].type in ['monetary', 'float']:
            classes += ['number']
            def formatter(v): return self.format_value_usd(v)
    ...
    def _get_query_period_table(self, options):
        ...
        period_values = [
            ...
            (minus_days(date, 16),   minus_days(date, 21)),
            (minus_days(date, 22),   minus_days(date, 28)),
            (minus_days(date, 29),  False),
        ]
    ...
    def _get_sql(self):
        ...
        query = ("""
            SELECT
                ...
                THEN %(sign)s * ROUND((
                    account_move_line.balance_usd - COALESCE(SUM(part_debit.amount), 0) + COALESCE(SUM(part_credit.amount), 0)
            ...
            HAVING ROUND(account_move_line.balance_usd - COALESCE(SUM(part_debit.amount), 0) + COALESCE(SUM(part_credit.amount), 0), currency_table.precision) != 0
        """).format(
    ...
    def _get_column_details(self, options):
        return [
            self._header_column(),
            self._field_column_usd('report_date'),
            self._field_column_usd('journal_code', name="Journal"),
            self._field_column_usd('account_name', name="Account"),
            self._field_column_usd('expected_pay_date'),
            self._field_column_usd('period0', name=_("As of: %s") % format_date(self.env, options['date']['date_to'])),
            self._field_column_usd('period1', sortable=True),
            self._field_column_usd('period2', sortable=True),
            self._field_column_usd('period3', sortable=True),
            self._field_column_usd('period4', sortable=True),
            self._field_column_usd('period5', sortable=True),
            self._custom_column(  # Avoid doing twice the sub-select in the view
                name=_('Total'),
                classes=['number'],
                formatter=self.format_value_usd,
...
class ReportAccountAgedReceivable(models.Model):
    _name = "account.aged.receivable.usd"
    _description = "Aged Receivable"
    _inherit = "account.aged.partner.usd"
    _auto = False
...
class ReportAccountAgedPayable(models.Model):
    _name = "account.aged.payable.usd"
