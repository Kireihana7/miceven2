...
class AccountInvoice(models.Model):
    ...
    def _compute_wh_iva(self):
        ...
        for invoice in self:
            if invoice.wh_id:
                ...
                invoice.amount_wh_iva = round(amount_iva, 4) if invoice.currency_id == self.env.company.currency_id else round(amount_iva, 4) * round(invoice.manual_currency_exchange_rate,4)
            else:
    ...
    def create_lines_retention(self, retention_id):
        if not self.can_delete_iva or not retention_id.asiento_iva:
            account_ret = self.company_id.sale_iva_ret_account.id if self.move_type in ('out_invoice', 'in_refund', 'out_receipt') else self.company_id.purchase_iva_ret_account.id
            vals = {
            ...
            amount_currency = retention_id.total_tax_ret * self.manual_currency_exchange_rate
            move_advance = {
                ...
                'credit': retention_id.total_tax_ret if self.move_type in ('out_invoice', 'in_refund', 'out_receipt') else 0.0,
                'debit': retention_id.total_tax_ret if self.move_type not in ('out_invoice', 'in_refund', 'out_receipt') else 0.0,
            ...
            asiento['debit'] = retention_id.total_tax_ret if self.move_type in ('out_invoice', 'in_refund', 'out_receipt') else 0.0
            asiento['credit'] = retention_id.total_tax_ret if self.move_type not in ('out_invoice', 'in_refund', 'out_receipt') else 0.0
            asiento['amount_currency']  = (amount_currency) if self.move_type in ('out_invoice', 'in_refund', 'out_receipt') else (amount_currency) *-1
    ...
    def action_post_iva(self):
        for inv in self:
            if inv.wh_id and inv.wh_id.state == 'draft':
                ...
                inv.wh_id.payment_id.action_post()
                pay_term_lines = inv.line_ids\
                ...
                lines=[]
                for lined in self.env['account.move.line'].search(domain,order='id desc',limit=1): 
                    lines = self.env['account.move.line'].browse(lined.id)
