...
class AccountWhIvaPay(models.Model):
    ...
    iva_partner_id = fields.Many2one('res.partner', string='Administración Tributaria', required=True, default=lambda self: self.env.user.company_id.iva_partner_id)
            ...
            for wh in wh_iva:
                ...
                for whl in wh.wh_lines:
                    if whl.state == 'declared':
                        total_amount += whl.ret_amount if wh.move_type in ('out_invoice','in_invoice') else -whl.ret_amount
    ...
    def pay_iva_withhold(self):
        ...
        self.name=self.env['ir.sequence'].next_by_code('account.payment.iva.in_invoice')
        for pay in self:
            amount_currency = sum(pay.wh_ids.filtered(lambda x: x.move_type in ('out_invoice','in_invoice')).mapped(lambda l: l.manual_currency_exchange_rate * l.total_tax_ret)) -  sum(pay.wh_ids.filtered(lambda x: x.move_type in ('out_refund','in_refund')).mapped(lambda l: l.manual_currency_exchange_rate * l.total_tax_ret))
            amount = sum(pay.wh_ids.filtered(lambda x: x.move_type in ('out_invoice','in_invoice')).mapped('total_tax_ret')) - sum(pay.wh_ids.filtered(lambda x: x.move_type in ('out_refund','in_refund')).mapped('total_tax_ret'))
            tasa = amount_currency / amount
            payment_dict = {
                ...
                'partner_id':pay.iva_partner_id.id,
            }
            ...
            #amount_currency = sum(pay.wh_ids.mapped(lambda l: l.manual_currency_exchange_rate * l.total_tax_ret))
            if credit_account_id:
                credit_line = (0, 0, {
                    'name': 'Pago del IVA retenido',
                    'account_id': credit_account_id,
