...
class ProductProduct(models.Model):
    ...
    def set_sale_quantity(self, quantity=0, type: str = "manual"):
        quantity = quantity or self.sudo().sale_quantity

        if type != "manual":
            decimals = float("{:.2f}".format(quantity % 1))

            DIGITS = {
                "floor": math.floor(quantity),
                "ceil": (math.ceil(quantity / .25) * .25),
            }

            if type == "increment":
                quantity = (DIGITS["floor"] + 1) if decimals >= 0.5 else (DIGITS["ceil"] + .25)
            else:
                quantity = (DIGITS["floor"] - 0.5) if ((decimals > 0.5) or (decimals == 0)) else (DIGITS["ceil"] - .25)

        values = []
        saleorder = self.sudo().env['sale.order'].browse(self.env.context.get('active_ids'))
        for product in self:
        ...
        self.sudo().sale_quantity = quantity
        return True

    def sale_manual_quantity(self):
        return self.sudo().set_sale_quantity()

    def sale_add_quantity(self):
        return self.sudo().set_sale_quantity(type="increment")

    def sale_remove_quantity(self):
        return self.sudo().set_sale_quantity(type="decrement")
