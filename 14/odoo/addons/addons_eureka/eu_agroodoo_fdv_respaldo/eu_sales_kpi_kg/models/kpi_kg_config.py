...
class KpiKgConfig(models.Model):
    ...
    start_date = fields.Date("Fecha de inicio", compute="_compute_dates", tracking=True,)
    end_date = fields.Date("Fecha de culminación", compute="_compute_dates", tracking=True,)
    salesperson_count = fields.Integer("Vendedores", compute='_compute_salesperson_count')
    branch_count = fields.Integer("Sucursales", compute='_compute_branch_count')
    category_count = fields.Integer("Categorías", compute='_compute_category_count')
    ...
    @api.depends("kpi_salesperson_id.goal")
