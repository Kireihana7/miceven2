   ...
   # @api.onchange('telf_hab_mother','telf_mov_mother')
    def onchange_phone_number(self, phone, field):
           ...
           if phone:
                ...
                if not res:
                    raise ValidationError('El número telefónico tiene el formato incorrecto. Ej: 0416-4567890. Por favor intente de nuevo')
           return {'value': res}
    ...
    def create(self, vals):
        ...
        if vals.get('mother_ci'):
            ...
            if not res:
                raise ValidationError ('La Cédula de la Madre debe contener solo números. Ej. 6231987')
        if vals.get('father_ci'):
            res = self.validate_father_ci(vals.get('father_ci'))
            if not res:
                raise ValidationError ('La Cédula del Padre debe contener solo números. Ej. 7190364')
        if vals.get('spouse_ci'):
            res = self.validate_spouse_ci(vals.get('spouse_ci'))
            if not res:
                raise ValidationError ('La Cédula del Conyuge debe contener solo números. Ej. 16654234')
        if vals.get('telf_hab_mother'):
            res = self.validate_phone_number(vals.get('telf_hab_mother'),'telf_hab_mother')
            if not res:
                raise ValidationError ('El número telefónico tiene el formato incorrecto. Ej: 0212-4567890. Por favor intente de nuevo')
    ...
    def write(self, vals):
        ...
        if vals.get('father_ci'):
            res = self.validate_father_ci(vals.get('father_ci'))
            if not res:
                raise ValidationError('La Cédula del Padre debe contener solo números. Ej. 11190364')
        if vals.get('spouse_ci'):
            res = self.validate_spouse_ci(vals.get('spouse_ci'))
            if not res:
                raise ValidationError('La Cédula del Conyuge debe contener solo números. Ej. 16654234')
...
class HrSon(models.Model):
    ...
    def _calculate_date_of_birth(self, value):
            ...
            if value:
                ahora = str(date.today())
                age = relativedelta.relativedelta(datetime.strptime(ahora, DEFAULT_SERVER_DATE_FORMAT),
                                                  datetime.strptime(str(value), DEFAULT_SERVER_DATE_FORMAT))
            return age
    @api.constrains('ci_sons')
    def validate_ci_sons(self):
        for rec in self:
            ci_obj = re.compile(r"""^\d{7,15}""", re.X)
            
            if rec.ci_sons and not ci_obj.search(rec.ci_sons):
                raise ValidationError('La Cédula del hijo debe contener solo números. Ej. 26231987')
    ...
    def write(self, vals):
        ...
        if vals.get('telf_hab_son'):
            ...
            if not res:
                raise ValidationError ('El Telefono de Habitación del Hijo tiene el formato incorrecto. Ej: 0212-4567890. Por favor intente de nuevo')
        if vals.get('telf_mov_son'):
            res = self.validate_phone_number(vals.get('telf_mov_son'), 'telf_mov_son')
            if not res:
                raise ValidationError ('El Telefono Móvil del Hijo tiene el formato incorrecto. Ej: 0416-4567890. Por favor intente de nuevo')
