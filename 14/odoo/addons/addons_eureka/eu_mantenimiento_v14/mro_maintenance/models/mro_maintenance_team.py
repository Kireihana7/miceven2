...
class MroMaintenanceTeam(models.Model):
    ...
    @api.depends('order_ids.maintenance_team_id', 'order_ids.state', 'order_ids.date_planned')
    def _compute_order_count(self):
        MroOrder = self.env['mro.order']
        result = {wid: {} for wid in self.ids}
        #Count Late Orders
        data = MroOrder.read_group([('maintenance_team_id', 'in', self.ids), ('state', 'in', ('released', 'ready', 'draft')), ('date_planned', '<', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))], ['maintenance_team_id'], ['maintenance_team_id'])
        ...
        res = MroOrder.read_group(
            ...
            ['maintenance_team_id', 'state', 'date_planned'], ['maintenance_team_id', 'state'],
            lazy=False)
