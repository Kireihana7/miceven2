odoo.define('hr_attendance_geofence.GeofenceDrawing', function (require) {
    ...
    var GeofenceDrawing = AbstractField.extend(GeofenceCommon.GeofenceCommon, {
        ...
        _make_area: function (center_lat, center_lon, p_radius){
            const coords = [center_lon, center_lat];
            const accuracy = ol.geom.Polygon.circular(coords, p_radius);
            this.vectorSource.clear(true);
            this.vectorSource.addFeatures([
                new ol.Feature(accuracy.transform('EPSG:4326', this.olmap.getView().getProjection())),
                new ol.Feature(new ol.geom.Point(ol.proj.fromLonLat(coords)))
            ]);
            this.olmap.getView().fit(this.vectorSource.getExtent(), {duration: 100, maxZoom:6});                   
            this.olmap.updateSize();
        },
    });
