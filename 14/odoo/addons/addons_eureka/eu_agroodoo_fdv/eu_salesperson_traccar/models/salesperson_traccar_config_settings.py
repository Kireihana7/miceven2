...
class SalespersonTraccarConfigSettings(models.Model):
    _name = "salesperson.traccar.config.settings"
    ...
    def _check_is_selected(self):
        active = self.env['salesperson.traccar.config.settings'].search_count([('active', '=', True)])
        if active > 1:
        ...
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'salesperson.traccar.config.settings', sequence_date=seq_date) or _("New")
