...
class SalespersonTraccarRouteDetails(models.Model):
    _name = "salesperson.traccar.route.details"
    _inherit = ['mail.thread']
    _description = "Route Details"
    _order = 'id desc'

    def _default_route_name(self):
        return self.env['ir.sequence'].next_by_code('salesperson.traccar.route.details')

    name = fields.Char(string='Name', default=lambda self: self._default_route_name(), help="Sequence of Route")
    employee_id = fields.Many2one('hr.employee', string='Empleado')
    traccar_device_uniqueid = fields.Char(related="employee_id.traccar_device_uniqueid", string="Traccar Unique Id", readonly=False)
    ...
    def write(self, vals):
        for obj in self:
            if not obj.name:
                vals['name'] = self.env['ir.sequence'].next_by_code('salesperson.traccar.route.details')
    ...
    def create_route(self, route_data): 
        route_details_sudo = self.env['salesperson.traccar.route.details'].sudo()
        hr_employee_sudo = self.env['hr.employee'].sudo()
        for route in route_data:
            ...
            hr_employee = hr_employee_sudo.search([('traccar_device_id', '=', device_id)], limit=1)            
            if hr_employee and hr_employee.is_traccar:                
                data['employee_id'] = hr_employee.id
            else:
                continue
