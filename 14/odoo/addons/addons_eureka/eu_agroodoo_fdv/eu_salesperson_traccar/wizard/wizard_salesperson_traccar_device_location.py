...
class WizardTraccarDeviceLocation(models.TransientModel):
    _name = "wizard.salesperson.traccar.device.location"
    _description = "Salesperson Traccar Device location Wizard"
    ...
    def create_location(self, location_data):
        if len(location_data) > 0:
            ...
            for location in location_data:
                ...
                locations = self.env['wizard.salesperson.traccar.current.position'].create(data)
            return locations
        else:
            return False
        ...
        if device_id:
            try:                
                ...
                if response.status_code == 200:
                    ...
                    if locations:
                        form_id = self.env.ref('eu_salesperson_traccar.form_details_salesperson_traccar_current_position').id
                        return {
                            'name': ("View Device Current Position"),
                            'type': 'ir.actions.act_window',
                            'res_model': 'wizard.salesperson.traccar.current.position',
                            'view_mode': 'form',
                            'views': [(form_id, 'form')],
...
class WizardTraccarCurretPosition(models.TransientModel):
    _name = "wizard.salesperson.traccar.current.position"
    _description = "Salesperson Traccar Device Current Position"
