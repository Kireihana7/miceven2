...
class KpiConfigZone(models.Model):
    _name = "kpi.config.zone"
    _description = " Zonas del KPI por kilogramo"
    _inherit = 'kpi.mixin'

    zone_id = fields.Many2one("res.partner.zones", "Zona",tracking=True,)
    name = fields.Char("Nombre", related="zone_id.name", tracking=True,)
    ...
    def create(self, vals_list):
        ...
        for rec in res:
            gerente_id = rec.kpi_salesperson_ids.create({
                "salesperson_id": rec.zone_id.manager_id.id,
