...
class Contract(models.Model):
    ...
    night_bonus_value = fields.Char(string='Night Bonus Value',  help="Acepta valores entre 00:00 y 23:59")
    night_bonus = fields.Float('Horas nocturnas', digits=(10,2), store=True)
    night_bonus_amount = fields.Float('Valor Horas bono nocturno')
    ...
    viatico_check = fields.Boolean('viatico')
    viatico_value = fields.Float('Viatico Value', digits=(10,2))
    seguro_hcm_check = fields.Boolean('Seguro HCM')
    seguro_hcm_value = fields.Float('Seguro HCM Value', digits=(10,2))
    special_bonus = fields.Boolean(string='Saturdays')
    ...
    diurnal_bonus_check = fields.Boolean(string='Diurnal Extra Hours')
    diurnal_extra_hours_value = fields.Char(string='Diurnal Extra Hours Value', size=5, help="Acepta valores entre 00:00 y 23:59")
    diurnal_extra_hours = fields.Float(string='Diurnal Extra Hours',digits=(10,2), store=True)
    diurnal_bonus_amount = fields.Float('Valor horas bono diurno')
 ...
 #   salary_assignment_check = fields.Boolean(string='Salary Assignment')
  #  salary_assignment_value = fields.Integer(string='Salary Assignment Value')
    ...
    hours_not_worked_value = fields.Char(string='Hours not Worked Value', size=5, help="Acepta valores entre 00:00 y 23:59")
    ...
    bonificacion_value = fields.Float()
    bono_salud=fields.Float("Bono salud")
    bono_capacitacion=fields.Float("Bono capacitación")

  
    # @api.multi no es necesario
    ...
    def _validate_changed_fields(self, values, come_from):
        ...
        #3##############################HORAS EXTRA DIURNO###################################

        diurnal_bonus_check = values.get('diurnal_bonus_check', False)
        if diurnal_bonus_check:
            valid_value = True
        else:
            valid_value = False
    ...
    # @api.multi no necesario en Odoo 13
    def _restore_all_fields(self, ids):
        ...
        for contract in self.browse(contract_id[0]):
            ...
            if contract.night_bonus_check:
                contract_fields.update({'night_bonus_check':False,'night_bonus_value': '0','night_bonus':0.0})
            ...
            if contract.diurnal_bonus_check:
                contract_fields.update({'diurnal_bonus_check':False,'diurnal_extra_hours_value':'0','diurnal_extra_hours':0.0})
            if contract.salary_retroactive_check:
                contract_fields.update({'salary_retroactive_check':False,'salary_retroactive_value':0.0})
            if contract.reposo_33_check:
                contract_fields.update({'reposo_33_check': False})
            if contract.special_bonus:
                contract_fields.update({'special_bonus': False, 'saturdays_value': '0'})
            if contract.viatico_check:
                contract_fields.update({'viatico_check': False, 'viatico_value': 0.0})
            ...
            if contract.seguro_hcm_check:
                contract_fields.update({'seguro_hcm_check': False, 'seguro_hcm_value': 0.0})


            ######configuracion de las vacaciones
            if contract.comision_check:
                contract_fields.update({'comision_check': False})
            if contract.salary_assignment_check:
                contract_fields.update({'salary_assignment_check': False, 'salary_assignment_value': 0.0})
            if contract.otras_asig_check:
                contract_fields.update({'otras_asig_check': False, 'otras_asig_value': 0.0})
            if contract.otras_ded_check:
                contract_fields.update({'otras_ded_check': False, 'otras_ded_value': 0.0})
    ...
    def _onchange_hours(self, value, field, come_from=None):
        ...
        if 'float' not in field:
            if value:
                ...
                if not res:
                    raise ValidationError('El formato de las horas es incorrecto, Solo acepta valores entre 00:00 y 23:59..\n'
                ...
                if come_from:
                    raise ValidationError('Ha seleccionado el campo %s, pero no ha introducido ningún valor.\n'
