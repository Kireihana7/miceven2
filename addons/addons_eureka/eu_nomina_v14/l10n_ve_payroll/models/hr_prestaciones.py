...
class HRPrestaciones(models.Model):
    _name = 'hr.prestaciones'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name=fields.Char(default=f"Prestaciones {TODAY.strftime('%B de %Y')}",tracking=True)
    employee_id=fields.Many2many('hr.employee',relation='employee_prestacion',column1="empleado",column2="prestacion",string="Empleados", domain=[('contract_id.state','in',['open','draft'])],tracking=True)
    ...
    def _get_nominas(self,empleado):
        return self.env['hr.payslip'].search([('is_anihilation','=',False),('employee_id','=',empleado.id)]).filtered(lambda x:x.date_to.month==self.fecha.month)
    def di_calculations(self):
        for rec in self:
            ...
            for empleado in rec.employee_id:
                ...
                prestaciones_lines=rec.env["hr.prestaciones.employee.line"].search([('line_type','=','prestacion'),('parent_id','=',empleado.id),('company_id','=',empleado.company_id.id)])
...
class HRAnticipos(models.Model):
    ...
    employee_id=fields.Many2many('hr.employee',relation='employee_anticipo',column1="empleado",column2="anticipo",string="Empleados", domain=['|',('mes_antig','>',3),('ano_antig','>',0)],tracking=True)
    fecha=fields.Date(default=date(TODAY.year,TODAY.month,monthrange(TODAY.year, TODAY.month)[1]),tracking=True)
    department_id=fields.Many2one('hr.department', "Departamento",tracking=True)
    cargo_id=fields.Many2one('hr.job',"Cargo",tracking=True)
    ...
    def get_anticipo_per_employee(self,employee_id):
        for rec in self:
            return rec.env["hr.prestaciones.employee.line"].search([('line_type','=','anticipo'),('parent_id','=',employee_id.id),('parent_lote_anticip','=',rec.id)])
