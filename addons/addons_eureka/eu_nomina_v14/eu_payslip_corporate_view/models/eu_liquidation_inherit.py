    ...
    def _compute_salario_promedio(self):
        for rec in self:
            if rec.fecha:
                ...
                nominas=rec.env['hr.payslip'].sudo().search([('corporate_type','=',False),('employee_id', '=', rec.employee_id.id),('date_from', '>=', begintime),('date_to', '<=', rec.fecha),('is_vacation','=',False),('is_anihilation','=',False),('is_utility','=',False),('type_struct_category','in',['normal','especial'])])

            else:
                begintime=TODAY -relativedelta(months=3)
                nominas=rec.env['hr.payslip'].sudo().search([('corporate_type','=',False),('employee_id', '=', rec.employee_id.id),('date_from', '>=', begintime),('date_to', '<=', TODAY),('is_vacation','=',False),('is_anihilation','=',False),('is_utility','=',False),('type_struct_category','in',['normal','especial'])])
    ...
    def _get_sal_diario(self):
        for rec in self:
                if rec.vacation_payslip:
                    nominas=rec.env['hr.payslip'].search([('corporate_type','=',False),('is_anihilation','=',False),('employee_id','=',rec.employee_id.id)]).filtered(lambda x:x.date_to.month==rec.fecha.month and x.id!=rec.vacation_payslip.id)
                else:
                    nominas=rec.env['hr.payslip'].search([('corporate_type','=',False),('is_anihilation','=',False),('employee_id','=',rec.employee_id.id)]).filtered(lambda x:x.date_to.month==rec.fecha.month)
                ...
                if total<contrato.complemento:
                    sal_mensual=contrato.complemento
                else:
                ...
                if sal_dia_total<1:
                    rec.sal_diario=rec.contract_id.complemento/30
                else:
                    rec.sal_diario=round(sal_dia_total,4)

class HRLiquidacionCorporate(models.Model):
    _name = 'hr.liquidacion.corporate'
    ...
    cut_in_half=fields.Boolean('Cortar a la mitad',tracking=True)
    ...
    lote_id=fields.Many2one("hr.liquidacion.corporate.lote",string="lote de liquidaciones")
    ...
    #Prestaciones
    Last_prestacion=fields.Many2one("hr.prestaciones.corporate.employee.line",compute="_get_last_prestacion_line",string="Ultima prestación",tracking=True)
    ...
    struct_vacaciones=fields.Many2one("hr.payroll.structure",default=lambda self: self.env.company.struct_vacacion_corporate,tracking=True)
    struct_liqidation=fields.Many2one("hr.payroll.structure",default=lambda self: self.env.company.struct_liquidacion_corporate,tracking=True)
    ...
    liquidado=fields.Many2one('hr.prestaciones.corporate.employee.line',"linea de liquidación",tracking=True)
    ...
    def _compute_salario_promedio(self):
        for rec in self:
            if rec.fecha:
                begintime=rec.fecha -relativedelta(months=3)
                nominas=rec.env['hr.payslip'].sudo().search([('corporate_type','=',True),('employee_id', '=', rec.employee_id.id),('date_from', '>=', begintime),('date_to', '<=', rec.fecha),('is_vacation','=',False),('is_anihilation','=',False),('is_utility','=',False),('type_struct_category','in',['normal','especial'])])

            else:
                begintime=TODAY -relativedelta(months=3)
                nominas=rec.env['hr.payslip'].sudo().search([('corporate_type','=',True),('employee_id', '=', rec.employee_id.id),('date_from', '>=', begintime),('date_to', '<=', TODAY),('is_vacation','=',False),('is_anihilation','=',False),('is_utility','=',False),('type_struct_category','in',['normal','especial'])])
            sal=sum(sum(y.total for y in x.line_ids.filtered(lambda line: line.category_id.code == 'BASIC')) for x in nominas)
    ...
    def _get_vacaciones_disfrutada(self):
        for rec in self:
            vacas=rec.env["hr.payslip"].search([('corporate_type','=',True),('is_vacation','=',True),('employee_id','=',rec.employee_id.id),('state','not in',['cancel','draft'])])
    ...
    def _get_dias_vacaciones_utili(self):
        for rec in self:
            ...
            ultima_utility=rec.env["hr.payslip"].search([('corporate_type','=',True),('employee_id','=',rec.employee_id.id),('is_utility','=',True),('state','not in',['cancel'])])
    ...
    def _get_last_prestacion_line(self):
        for rec in self:
            rec.Last_prestacion= rec.env["hr.prestaciones.corporate.employee.line"].search([('parent_id','=',rec.employee_id.id)],limit=1,order="id desc")

    @api.depends("employee_id","fecha")
    def _get_sal_diario(self):
        for rec in self:
                if rec.vacation_payslip:
                    nominas=rec.env['hr.payslip'].search([('corporate_type','=',True),('is_anihilation','=',False),('employee_id','=',rec.employee_id.id)]).filtered(lambda x:x.date_to.month==rec.fecha.month and x.id!=rec.vacation_payslip.id)
                else:
                    nominas=rec.env['hr.payslip'].search([('corporate_type','=',True),('is_anihilation','=',False),('employee_id','=',rec.employee_id.id)]).filtered(lambda x:x.date_to.month==rec.fecha.month)
                total=sum(x.total  for x in nominas.mapped("line_ids").filtered(lambda x: x.category_id.code=="BASIC"))
                ...
                if total<contrato.complemento:
                    ...
                    sal_mensual=total #obtenido sal_mensual
                sal_diario=sal_mensual/30 #obtenido sal_diario
                rec.alicuota_utilidades=0 #(sal_diario*rec.env.company.cant_dias_utilidades)/360 #obtenido utilidades 
                dias_vacaciones=rec.env.company.dias_vac_base + rec.employee_id.dias_bono
                rec.alicuota_vacaciones=0 #(sal_diario*dias_vacaciones)/360 #obtenido vacaciones
                sal_dia_total=sal_diario+rec.alicuota_vacaciones+rec.alicuota_utilidades #tenemos diario integral
                if sal_dia_total<1:
                    rec.sal_diario=rec.contract_id.complemento/30
                else:
    ...
    @api.depends('total_liquidar','prestacion_total') #'minimo_a_liquidar'
    ...
    def liquidar(self):
        for rec in self:
            ...
            liquidanom.input_line_ids[0].amount=rec.total_utilidades if not rec.cut_in_half else rec.total_utilidades/2
            liquidanom.input_line_ids[1].amount=rec.intereses_prestaciones if not rec.cut_in_half else rec.intereses_prestaciones/2
            liquidanom.input_line_ids[2].amount=rec.prestacion_total if not rec.cut_in_half else rec.prestacion_total/2
            # liquidanom.input_line_ids[3].amount=rec.minimo_a_liquidar
            if rec.cut_in_half:
                liquidanom.input_line_ids[4].amount=rec.monto_art92_liq/2 if rec.motivo=="despido" else 0
            else:
                liquidanom.input_line_ids[4].amount=rec.monto_art92_liq if rec.motivo=="despido" else 0
            liquidanom.input_line_ids[5].amount=rec.adicionales if not rec.cut_in_half else rec.adicionales/2
            ...
            rec.great_sumatory=(rec.total_utilidades+rec.intereses_prestaciones+rec.total_liquidar)/(2 if rec.cut_in_half else 1)
            rec.liquidado=rec.env['hr.prestaciones.corporate.employee.line'].create({
                    ...
                    'tasa_activa':False,
                    'intereses':False,
                    'intereses_acum':rec.intereses_prestaciones if not rec.cut_in_half else rec.intereses_prestaciones/2,
                    'presta_e_inte':(rec.intereses_prestaciones+rec.total_liquidar)/(1 if not rec.cut_in_half else 2),
...
class HRLiquidacionCorporateLote(models.Model):
    _name="hr.liquidacion.corporate.lote"
