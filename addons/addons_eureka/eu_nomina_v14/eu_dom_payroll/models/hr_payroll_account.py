...
class HrPayslip(models.Model):
    ...
    def action_payslip_done(self):
        ...
        for slip in self:
            if slip.line_ids.filtered(lambda x:x.code=='PRESTAMO'):
                ...
                for loan in slip.employee_id.loan_ids.filtered(lambda loan: loan.state == 'approved'):
                    if slip.pay_vacation and slip.vacation_type == 'enjoyed' and 13 <= payslip_day <= 16:
                        ...
                        loan_line = loan.loan_line_ids.filtered(lambda loan_line: loan_line.date<=slip.date_to and loan_line.number == loan.next_fee)
                        if loan_line:
