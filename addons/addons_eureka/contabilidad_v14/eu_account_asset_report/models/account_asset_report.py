...
class assets_report(models.AbstractModel):
    ...
    _name = 'account.assets.report.ved'
    ...
    def _get_report_name(self):
        return _('Programa de Depreciación')

    def _get_templates(self):
        templates = super(assets_report, self)._get_templates()
        templates['main_template'] = 'eu_account_asset_report.main_template_asset_report_ved'
        return templates
    ...
    def get_header(self, options):
        ...
        return [
            [
                {'name': ''},
                {'name': _('Características'), 'colspan': 4},
                {'name': _('Activo'), 'colspan': 4},
                {'name': _('Depreciación'), 'colspan': 4},
                {'name': _('Valor en Libros')},
            ],
            [
                {'name': ''},  # Description
                {'name': _('Fecha de Adquisición'), 'class': 'text-center'},  # Characteristics
                {'name': _('Primera Depreciación'), 'class': 'text-center'},
                {'name': _('Método'), 'class': 'text-center'},
                {'name': _('Tasa'), 'class': 'number', 'title': _('In percent.<br>For a linear method, the depreciation rate is computed per year.<br>For a declining method, it is the declining factor'), 'data-toggle': 'tooltip'},
