...
class AccountWhIva(models.Model):
    ...
    def _get_amount_total(self):
        for wh in self:
            wh.total_tax_ret = round(sum(wh.wh_lines.filtered(lambda whl: whl.state != 'annulled').mapped('ret_amount')), 4)
    ...
    def act_getfile(self,wh_id,tax_period):
        ...
        rs=''
        type_doc = ''
        if not self.env.company.rif:
            raise UserError('El RIF para la compañía no ha sido establecido.')
        rif = r.join(str(wh_id.mapped('company_id.rif')).split('-'))
        content = ''
        for wh in wh_id:
            ...
            for whl in wh.wh_lines:
                ...
                if whl.state != 'annulled':
                    ...
                    whl.invoice_id.nro_control if whl.invoice_id.parent_id else whl.invoice_id.nro_control,
                    whl.invoice_id.amount_total * whl.invoice_id.manual_currency_exchange_rate if whl.invoice_id.currency_id == whl.invoice_id.company_id.currency_id else whl.invoice_id.amount_total,
                    base_imponible* whl.invoice_id.manual_currency_exchange_rate,
                    whl.ret_amount * whl.invoice_id.manual_currency_exchange_rate,
                    whl.invoice_id.parent_id.vendor_invoice_number if whl.invoice_id.parent_id else whl.invoice_id.debit_origin_id.vendor_invoice_number if whl.invoice_id.debit_origin_id else zero,
                    wh.number,
...
class AccountWhIvaLine(models.Model):
    ...
    def _get_ret_amount(self):
        for wh in self:
            ...
            wh.ret_amount = round((wh.rate_amount * wh.amount_tax / 100), 4)

    @api.model
    def _get_sub_total(self):
        for wh in self:
            wh.sub_total = round(wh.base_tax + wh.amount_tax, 4)
