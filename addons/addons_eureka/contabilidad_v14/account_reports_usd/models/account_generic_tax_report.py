...
class generic_tax_report(models.AbstractModel):
    ...
    _name = 'account.generic.tax.report.usd'
        ...
        sql = """
            SELECT id, account_id, sum(tax) AS tax, sum(net) AS net FROM (
                SELECT tax.id,
                       %(select_account)s
                       0.0 AS tax,
                       "account_move_line".balance_usd AS net
                ...
                SELECT tax.id,
                       %(select_account)s
                       "account_move_line".balance_usd AS tax,
                       0.0 AS net
    ...
    def _sql_tax_amt_regular_taxes(self, group_by_account=False):
        sql = """
            SELECT "account_move_line".tax_line_id,
                   %(select_account)s
                   COALESCE(SUM("account_move_line".balance_usd), 0)
              FROM account_tax tax, {tables}
             WHERE {where_clause}
    ...
    def _sql_net_amt_regular_taxes(self, group_by_account=False):
        sql = '''
            SELECT tax.id,
                   %(select_account)s
                   COALESCE(SUM("account_move_line".balance_usd))
              FROM {tables}
    ...
    def _postprocess_lines(self, lines, options):
        ...
        def assign_active_section(col_nber):
            ...
            line_to_assign['columns'] = [{'name': self.format_value_usd(balance), 'style': 'white-space:nowrap;', 'balance': balance} for balance in total_balance_col]
        ...
        for i, calc in enumerate(tax_report.get_checks_to_perform(mapping)):
            if calc[1]:
                if isinstance(calc[1], float):
                    value = self.format_value_usd(calc[1])
                else:
                    value = calc[1]
    ...
    def _build_total_line(self, report_line, balances_by_code, formulas_dict, hierarchy_level, number_periods, options):
        ...
        for period_index in range(0, number_periods):
            ...
            columns.append({'name': '' if period_total is None else self.format_value_usd(period_total), 'style': 'white-space:nowrap;', 'balance': period_total or 0.0})
    ...
    def _build_tax_grid_line(self, grid_data, hierarchy_level):
        ...
        for period in grid_data['periods']:
            columns += [{'name': self.format_value_usd(period['balance']), 'style': 'white-space:nowrap;', 'balance': period['balance']}]
    ...
    def _get_lines_by_tax(self, options, line_id, taxes):
        ...
        for line in lines:
            for column in line['columns']:
                column['name'] = self.format_value_usd(column['no_format'])
        return lines
