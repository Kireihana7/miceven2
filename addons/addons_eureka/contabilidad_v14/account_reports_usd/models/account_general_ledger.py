...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    def _get_options_domain(self, options):
        ...
        if options.get('filter_products'):
            domain += [
                '|',
                ('product_id.name', 'ilike', options['filter_products']),
                ('product_id.default_code', 'ilike', options['filter_products'])
            ]
        return domain
    ...
    def _get_account_title_line(self, options, account, amount_currency, debit, credit, balance, has_lines):
        ...
        columns = [
            {'name': self.format_value_usd(debit), 'class': 'number'},
            {'name': self.format_value_usd(credit), 'class': 'number'},
            {'name': self.format_value_usd(balance), 'class': 'number'},
        ]
        #if self.user_has_groups('base.group_multi_currency'):
        #    columns.insert(0, {'name': has_foreign_currency and self.format_value_usd(amount_currency, currency=account.currency_id, blank_if_zero=True) or '', 'class': 'number'})
        return {
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        ...
        columns = [
            ...
            {'name': self.format_value_usd(aml['debit'], blank_if_zero=True), 'class': 'number'},
            {'name': self.format_value_usd(aml['credit'], blank_if_zero=True), 'class': 'number'},
            {'name': self.format_value_usd(cumulated_balance), 'class': 'number'},
            {'name': aml['product_name'], 'title': aml['product_name'], 'class': 'whitespace_print'},
        ]
        #if self.user_has_groups('base.group_multi_currency'):
        #    columns.insert(3, {'name': currency and aml['amount_currency'] and self.format_value_usd(aml['amount_currency'], currency=currency, blank_if_zero=True) or '', 'class': 'number'})
        return {
            'id': aml['id'],
    ...
    def _get_account_total_line(self, options, account, amount_currency, debit, credit, balance):
        ...
        columns = []
        #if self.user_has_groups('base.group_multi_currency'):
        #    columns.append({'name': has_foreign_currency and self.format_value_usd(amount_currency, currency=account.currency_id, blank_if_zero=True) or '', 'class': 'number'})

        columns += [
            {'name': self.format_value_usd(debit), 'class': 'number'},
            {'name': self.format_value_usd(credit), 'class': 'number'},
            {'name': self.format_value_usd(balance), 'class': 'number'},
            {'name': 'Aquí Va un producto'},
        ]
