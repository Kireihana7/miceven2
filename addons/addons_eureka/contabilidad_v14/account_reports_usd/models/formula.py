...
class FormulaSolver:
    def __init__(self, options_list, financial_report):
        ...
        # A mapping of financial.report.line's code => account.financial.html.report.usd.line record
    ...
    def _eval_formula(self, financial_line, key):
        ...
        :param financial_line:  A record of the account.financial.html.report.usd.line model.
    ...
    def _get_line_by_code(self, line_code):
        ''' Retrieve an account.financial.html.report.usd.line record from its code.
        ...
        :param line_code:   The code that could be owned by the account.financial.html.report.usd.line record.
        :return:            An account.financial.html.report.usd.line recordset having 0 or 1 as arity.
        '''
        if line_code in self.cache_line_by_code:
            ...
            # Fetch line.
            financial_line = self.env['account.financial.html.report.usd.line'].search([('code', '=', line_code)], limit=1)
    ...
    def fetch_lines(self, financial_lines):
        ...
        '''
        children_financial_lines = self.env['account.financial.html.report.usd.line']
    ...
    def get_formula_string(self, financial_line):
        ...
        def inject_in_formula(formula, to_replace, to_write, is_monetary=False):
            ...
            if to_replace == 'sum' and is_monetary and to_write < 0.0 and re.sub(r'\s*', '', formula) == '-sum':
                return self.env['account.report'].format_value_usd(-to_write)

            if is_monetary:
                to_write = self.env['account.report'].format_value_usd(to_write)
