...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_report_line_partner(self, options, partner, initial_balance, debit, credit, balance):
        ...
        columns = [
            {'name': self.format_value_usd(initial_balance), 'class': 'number'},
            {'name': self.format_value_usd(debit), 'class': 'number'},
            {'name': self.format_value_usd(credit), 'class': 'number'},
        ]
        #if self.user_has_groups('base.group_multi_currency'):
        #    columns.append({'name': ''})
        columns.append({'name': self.format_value_usd(balance), 'class': 'number'})
    ...
    def _get_report_line_move_line(self, options, partner, aml, cumulated_init_balance, cumulated_balance):
        ...
        columns = [
            ...
            {'name': self.format_value_usd(cumulated_init_balance), 'class': 'number'},
        ...
        #        formatted_amount = self.format_value_usd(aml['amount_currency'], currency=currency, blank_if_zero=True)
        #        columns.append({'name': formatted_amount, 'class': 'number'})
        #    else:
        #        columns.append({'name': ''})
        columns.append({'name': self.format_value_usd(cumulated_balance), 'class': 'number'})
        return {
            'id': aml['id'],
