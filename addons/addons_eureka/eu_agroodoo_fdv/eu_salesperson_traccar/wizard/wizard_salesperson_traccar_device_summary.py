...
class WizardTraccarDeviceSummary(models.TransientModel):
    _name = "wizard.salesperson.traccar.device.summary"
    _description = "Salesperson Traccar Device Summary Wizard"
    ...
    def create_summary(self, summary_data):
        if len(summary_data) > 0:
            ...
            for summary in summary_data:
                ...
                data['name'] = "Salesperson Traccar Device Summary Details"
                ...
                details = self.env['details.salesperson.traccar.device.summary'].create(data)
            return details
        else:
            return False
        ...
        if device_id:
            try:                
                ...
                if response.status_code == 200:
                    ...
                    if details:
                        form_id = self.env.ref('eu_salesperson_traccar.form_details_salesperson_traccar_device_summary').id
                        return {
                            'name': ("View Device Summary Details"),
                            'type': 'ir.actions.act_window',
                            'res_model': 'details.salesperson.traccar.device.summary',
                            'view_mode': 'form',
                            'views': [(form_id, 'form')],
...
class DetailsTraccarDeviceSummary(models.TransientModel):
    _name = "details.salesperson.traccar.device.summary"
    _description = "Salesperson Traccar Device Summary Details"
