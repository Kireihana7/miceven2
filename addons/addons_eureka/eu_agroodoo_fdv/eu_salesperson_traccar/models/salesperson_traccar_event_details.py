...
class SalespersonTraccarEventDetails(models.Model):
    _name = "salesperson.traccar.event.details"
    _inherit = ['mail.thread']
    _description = "Event Details"
    _order = 'id desc'

    def _default_event_name(self):
        return self.env['ir.sequence'].next_by_code('salesperson.traccar.event.details')
    ...
    def write(self, vals):
        for obj in self:
            if not obj.name:
                vals['name'] = self.env['ir.sequence'].next_by_code('salesperson.traccar.event.details')
    ...
    def create_event(self, event_data): 
        event_details_sudo = self.env['salesperson.traccar.event.details'].sudo()
