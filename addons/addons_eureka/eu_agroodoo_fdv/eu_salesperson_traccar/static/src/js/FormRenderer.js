odoo.define('eu_salesperson_traccar.FormRenderer', function (require) {
    "use strict";
    ...
    var TraccarFormRenderer = FormRenderer.extend({
        cssLibs: [
            '/eu_salesperson_traccar/static/src/js/lib/ol-7.1.0/ol.css',
            '/eu_salesperson_traccar/static/src/css/ol_style.css',
        ],
        jsLibs: [
            '/eu_salesperson_traccar/static/src/js/lib/ol-7.1.0/ol.js',
        ],
        ...
        addRoutePoints: function(){
            ...
            if (self.state.data.src_latitude && self.state.data.src_longitude) {
                ...
                fetch(url).then(function (response) {
                    ...
                    rpc.query({
                        model: 'hr.employee',
                        method: 'get_loc_clients',
                        args: [data.employee_id.res_id]
                    }).then((locs) => {
                        for(let loc of locs) {
                            self.vectorSource.addFeature(self.addMarker(loc, true));
                        }
                    });
        ...
        addMarker: function(coords, is_client=false) {
            var self = this;
            let src = "/eu_salesperson_traccar/static/src/img/marker.png"
            if(is_client) {
                src = "/eu_salesperson_traccar/static/src/img/marker_client.png"
            }
            ...
            if(is_client) {
                desc = '<pre><span style="font-weight: bold;">' + coords.name + '</span>' + '<br>' + 'Latitude : ' + coords.lat + '<br>Longitude: ' + coords.long + '</pre>';
            }
            ...
            if (self.state.data.latitude && self.state.data.longitude) {
                ...
                var iconStyle = new ol.style.Style({
                    image: new ol.style.Icon({
                        scale: .2,
                        crossOrigin: 'anonymous',
                        src: "/eu_salesperson_traccar/static/src/img/marker.png",
                        opacity: 1,
                    })
                });
