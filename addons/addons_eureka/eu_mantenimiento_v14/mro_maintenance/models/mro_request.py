...
class MroRequest(models.Model):
    ...
    def action_send(self):
        for request in self:
            ...
            current_user_id = request.write_uid or request.create_uid
            if current_user_id.partner_id.email and request.requested_by.partner_id.email:
                request.sudo().request_for_approval_mail_send(current_user_id.partner_id.email, request.requested_by.partner_id.email)
        return True

    def request_for_approval_mail_send(self, email_from, email_to):
