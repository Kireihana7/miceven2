...
class MroMeter(models.Model):
    ...
    state = fields.Selection(STATE_SELECTION, 'Status', readonly=True, default='draft', copy=False)
    ...
    equipment_id = fields.Many2one('mro.equipment', 'Equipment', readonly=True, copy=False)
    measure_type = fields.Selection(MEASURE_TYPE_SELECTION, 'Measure Type', required=True, readonly=True, states={'draft': [('readonly', False)]}, default='delta')
    actual_utilization = fields.Float('Actual Utilization', compute='_get_utilization', store=True)
    date_meter_install = fields.Datetime('Meter installment date', readonly=True, states={'operative': [('readonly', False)]}, copy=False)
    ...
    @api.constrains('meter_uom')
    def onchange_uom(self):
        if self.meter_line_ids:
            raise UserError(_("Meter Measures have been entered, please delete them before"))
        return True
...
class MroMeterLine(models.Model):
    ...
    def check_date(self):
        if self.date and self.meter_id:
            if self.meter_id.date_meter_install and self.meter_id.date_meter_install >= self.date:
                raise UserError(_("Measure date is before the meter installment date"))
            if self.meter_id.date and self.meter_id.measure_type == 'up':
                if self.date < self.meter_id.date:
                    raise UserError(_("Measure date is before the last measure"))
    
    @api.constrains('value')
    def check_value(self):
        if self.meter_id and self.value < 0:
            raise UserError(_("Measure has to be a positive value"))
        if self.meter_id and self.meter_id.measure_type == 'up':
