...
class MroEquipment(models.Model):
    ...
    tag_ids = fields.Many2many('mro.equipment.tags', string='Tags')
    ...
    gauge_state = fields.Selection(related='gauge_id.state', store=True)
    ...
    meter_state = fields.Selection(related='meter_id.state', store=True)
    ...
    child_all_count = fields.Integer('All SubEquipments Count', compute='_compute_subequipments', compute_sudo=True, recursive=True)
    ...
    plan_meter_state = fields.Selection(related='plan_meter_id.state', store=True)
    ...
    # KPI
    mttr = fields.Integer('MTTR', compute='_compute_key_indicator', help='Mean Time To Repair, maintenance KPI based on done corrective maintenances.')
    mtbf = fields.Integer('MTBF', compute='_compute_key_indicator', help='Mean Time Between Failure, maintenance KPI based on done corrective maintenances.')
    lfd = fields.Date('Latest Failure Date', compute='_compute_key_indicator')
    enf = fields.Date('Estimated time before next failure', compute='_compute_key_indicator', help='Estimated days before next failure, maintenance KPI as Latest Failure Date + MTBF')

    @api.depends('order_ids.date_execution')
    def _compute_key_indicator(self):
        for equipment in self:
            # MTTR
            corrective_orders = equipment.order_ids.filtered(lambda x: x.maintenance_type == 'bm' and x.state == 'done')
            mttr_days = mtbf_days = mtbf_counter = 0
            for order in corrective_orders:
                mttr_days += (order.date_execution.date() - order.date_planned.date()).days
            equipment.mttr = len(corrective_orders) and (mttr_days / len(corrective_orders)) or 0
            # MTBF/LFD/ENF
            corrective_orders_sorted = corrective_orders.sorted(lambda x: x.date_execution)
            equipment.lfd = corrective_orders_sorted and corrective_orders_sorted[-1].date_execution or False
            equipment.mtbf = equipment.enf = False
            if len(corrective_orders_sorted) > 1:
                for i in range(len(corrective_orders_sorted) -1):
                    if corrective_orders_sorted[-i-1].date_planned > corrective_orders_sorted[-i-2].date_execution:
                        mtbf_days += (corrective_orders_sorted[-i-1].date_planned.date() - corrective_orders_sorted[-i-2].date_execution.date()).days
                        mtbf_counter += 1
                if mtbf_counter > 0:
                    equipment.mtbf = (mtbf_days/mtbf_counter)
            if equipment.mtbf:
                equipment.enf = equipment.lfd + relativedelta(days=equipment.mtbf)
    ...
    def action_plan_meter_line_equipment(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'res_model': 'mro.meter.line.wizard',
    ...
    def action_gauge_line_equipment(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'res_model': 'mro.gauge.line.wizard',
            'type': 'ir.actions.act_window',
            'target': 'new',
        }
    ...
    def action_reset_meter_data(self):
        for equi in self:
            equi.actual_last_utilization = 0.0
            equi.actual_last_maintenance = False
            equi.last_measure_date = False
        return True 
    ...
    def action_plan_meter_unassign(self):
        for equi in self:
            ...
            #    raise UserError(_("please detach the meter before unassigning it"))
            #else:
            equi.plan_meter_id.detact_meter()
    ...
    @api.depends('order_ids.state', 'order_ids.date_planned')
    def _next_maintenance(self):
        for equi in self:
            ...
            order_id = self.env['mro.order'].search(
                ...
                limit=1)
            if order_id:
                equi.maintenance_date = order_id.date_start_execution or order_id.date_planned 
        return True
...
class MroEquipmentTags(models.Model):
    _name = "mro.equipment.tags"
    _description = "MRO Equipemnt Tags"
...
class MeterAssign(models.TransientModel):
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env.company, readonly=True)
    resource_calendar_id = fields.Many2one('resource.calendar', string=_('Equipment Working Time'), domain="[('company_id','=', company_id)]", required=True)
    ...
    def do_assign(self):
        ...
        self.meter_id.install_meter()
        return True


class MeterLineWizard(models.TransientModel):
    _name = 'mro.meter.line.wizard'
    _description = "Wizard Meter Line"        
    
    meter_line_ids = fields.Many2many('mro.meter.line', string='Meter Lines')
    meter_id = fields.Many2one('mro.meter', 'Meter')
    ...
    def do_populate_meter_line_ids(self):
        self.ensure_one()
        selected_meter_line_ids = self.env['mro.meter.line'].search([('meter_id', '=', self.meter_id.id)])
        self.meter_line_ids = selected_meter_line_ids
        return self._reopen_form()
        
class GaugeLineWizard(models.TransientModel):
    _name = 'mro.gauge.line.wizard'
    _description = "Wizard Gauge Line"        
    
    gauge_line_ids = fields.Many2many('mro.gauge.line', string='Gauge Lines')
    gauge_id = fields.Many2one('mro.gauge', 'Gauge')
    ...
    def do_populate_gauge_line_ids(self):
        self.ensure_one()
        selected_gauge_line_ids = self.env['mro.gauge.line'].search([('gauge_id', '=', self.gauge_id.id)])
        self.gauge_line_ids = selected_gauge_line_ids
        return self._reopen_form()
