...
class MroOrder(models.Model):
    ...
    maintenance_priority = fields.Selection(PRIORITY_SELECTION, 'Maintenance Priority', default='0', required=True, 
        readonly=True, states={'draft': [('readonly', False)]})
    # dates
    date_planned = fields.Datetime('Requested Date', required=True, default=lambda self: fields.datetime.now())
    ...
    date_document = fields.Datetime('Document Date', default=lambda self: fields.datetime.now(), required=True, readonly=True, states={'draft': [('readonly', False)]})
    # text
    ...
    task_note = fields.Text('Technical Notes', related='task_id.note', readonly=True)
    # parts
    parts_stock_moves = fields.One2many('stock.move', 'maintenance_id', 'Parts Stock Moves')
    parts_stock_moves_availability = fields.Boolean('Spare Parts Moves Availability', compute='_get_moves_availability', store=True)
    ...
    warehouse_id = fields.Many2one('stock.warehouse', 'Warehouse', readonly=True,  states={'draft': [('readonly', False)]}, compute='_get_warehouse', store=True)
    location_parts_id = fields.Many2one('stock.location', 'Spare Parts Location', readonly=True, states={'draft': [('readonly', False)]}, compute='_get_location_parts_id', store=True)
    # operations
    ...
    activity = fields.Many2many('mro.activity', string='Operations', domain="[('check_list_id','=', check_list_id)]", 
        readonly=True, states={'released': [('readonly', False)]})
    ...
    def _get_location_parts_id(self):
        for order in self:
            if order.warehouse_id:
                order.location_parts_id = order.warehouse_id.lot_stock_id.id
                order.company_id = order.warehouse_id.company_id.id
            else:
                order.location_parts_id = False
        return True
    ...
    def action_confirm(self):        
        ...
        # active_orders = self.env['mro.order'].search([('state', '=', 'released')])
        tools = []
        ...
        for order in self:
            ...
            if any(tool.date_next_calibration and tool.date_next_calibration <= datetime.now() for tool in order.tool_ids):
            ...
            if len(order.parts_lines) > 0:
                if not order.warehouse_id:
                    raise UserError(_('please enter a Warehouse'))
                if not order.location_parts_id:
                    raise UserError(_('please enter a Location'))
                picking_type_id = self.env['stock.picking.type'].search([('code', '=', 'internal'),('warehouse_id', '=', order.warehouse_id.id)], limit=1).id
                if not picking_type_id:
                    raise UserError(_('no transfer picking type has not been found'))
                order._generate_parts_moves()
                order.check_availability()
            order.equipment_id.state = 'mn'
            order.date_start_execution = datetime.now()
            if order.tool_ids:
                for tool in order.tool_ids:
    ...
    def _generate_parts_moves(self):
        ...
        for order in self:
            for line in order.parts_lines:
                ...
                picking = self.env['stock.picking'].create(order.get_picking_values())
                order.parts_stock_moves.write({'picking_id': picking.id})                
        return True

    def get_picking_values(self):
        cons_loc_id = self.env['stock.location'].search([('usage', '=', 'equi')], limit=1).id
        picking_type_id = self.env['stock.picking.type'].search([('code', '=', 'internal'),('warehouse_id', '=', self.warehouse_id.id)], limit=1).id
        return {
            'origin': self.name,
    ...
    @api.depends('parts_stock_moves.state')
    def _get_moves_availability(self):
        availability = True
        states = []
        cons_loc_id = self.env['stock.location'].search([('usage', '=', 'equi')], limit=1).id
        for order in self:
            if order.parts_stock_moves:
                states += [move.state not in ('assigned','done','cancel') for move in order.parts_stock_moves if move.location_dest_id.id == cons_loc_id]
                if any(states):
                    availability = False
            order.parts_stock_moves_availability = availability
        return True
    ...
    def action_product_forecast_report(self):
        ...
        action['context'] = {
            ...
            'move_to_match_ids': self.parts_stock_moves.filtered(lambda m: m.product_id == self.product_id).ids
        }
...
class MroOrderPartsLine(models.Model):
    ...
    def action_product_forecast_report(self):
        self.ensure_one()
        action = self.parts_id.action_product_forecast_report()
        action['context'] = {
            'active_id': self.parts_id.id,
            'active_model': 'product.product',
        }
        warehouse = self.maintenance_id.warehouse_id
        if warehouse:
            action['context']['warehouse'] = warehouse.id
        return action
