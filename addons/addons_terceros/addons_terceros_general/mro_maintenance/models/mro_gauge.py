...
class MroGauge(models.Model):
    ...
    date_gauge_install = fields.Datetime('Gauge installment date', readonly=True, copy=False)
    doc_count = fields.Integer('Number of documents attached', compute='_compute_attached_docs_count')
    note = fields.Text('Internal Notes')


    @api.constrains('gauge_uom')
    def onchange_uom(self):
        if self.gauges_line_ids:
            raise UserError(_("Gauge Measures have been entered, please delete them before"))
        return True
