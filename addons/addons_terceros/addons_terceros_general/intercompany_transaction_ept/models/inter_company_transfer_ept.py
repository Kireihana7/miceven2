...
class InterCompanyTransferEpt(models.Model):
    ...
    currency_id = fields.Many2one(related="pricelist_id.currency_id",
                                  help="Currency of company or by pricelist.",tracking=True)
    ...
    stock_plus_count = fields.Float(string="Recepciones Adicionales",compute="_compute_stock_plus")
    stock_plus = fields.One2many('stock.picking', 'inter_company_transfer_id_new',copy=False,string="Recepciones Adicionales")

    def create_picking_intertransfer(self):
        # return {
        #     'name': _('Recepciones Adicionales'),
        #     'view_mode': 'tree',
        #     'view_type': 'tree',
        #     'type': 'ir.actions.act_window',
        #     'res_model': 'stock.picking',
        #     'view_id': False,
        #     'views': [(self.env.ref('stock.action_picking_tree_all').id, 'tree')],
        #     'domain':[]
        #     #'domain': [('inter_company_transfer_id_new', '=', self.id),('picking_type_id','in',self.env['stock.picking.type'].search([('code','=','incoming')]).mapped('id'))],
        #     #'context': {'default_inter_company_transfer_id_new':self.id},
        # }
        ...
        return {
            ...
            'res_model': 'stock.picking',
            'view_mode': 'tree',
            'view_id': self.env.ref('stock.action_picking_tree_all').id,
            'context': self.env.context,
            #'target': 'new',
            'type': 'ir.actions.act_window',
            #'domain': [('inter_company_transfer_id_new', '=', self.id)],
            #'context': {'default_inter_company_transfer_id_new':self.id},
        }
    
    @api.depends('stock_plus')
    def _compute_stock_plus_count(self):
        for rec in self:
            rec.stock_plus_count = len(rec.stock_plus)
    @api.depends('log_ids')
    ...
    def on_barcode_scanned(self, barcode):
        ...
        line = ict_line_obj.sudo().search([('inter_company_transfer_id', '=', self._origin.id),
                                    ('product_id', '=', product_id.id)], limit=1)
        if line:
            line.sudo().write({'quantity':line.quantity + 1})
        else:
            ict_line_obj.sudo().create({'inter_company_transfer_id':self._origin.id,
        ...
        if self.state == 'processed':
            ...
            if invoices.filtered(lambda x: x.state == 'post'):
                raise Warning("You can not cancel Inter Company Transfer which has posted "
                              "invoices.")
            self.sale_order_ids.sudo().action_cancel()
            self.purchase_order_ids.sudo().button_cancel()
            pickings.sudo().action_cancel()
            invoices.sudo().button_cancel()
    ...
    def create_internal_transfer(self):
        ...
        group_id = procurement_group_obj.sudo().create({'name': self.name,
        ...
        if procurements:
            self.env['procurement.group'].sudo().with_context(default_type=destination_warehouse.mapped('int_type_id.id')).run(procurements)

        pickings = self.env['stock.picking'].sudo().search([('group_id', '=', group_id.id)])
        if not pickings:
            raise Warning("No Pickings are created for this record.")
        pickings.sudo().write({'inter_company_transfer_id':self.id})
    ...
    def create_sale_order(self):
        ...
        for record in self:
            ...
            # Creates sale order.
            order_vals = sale_obj.sudo().new({'partner_id':customer_partner_id.id,
                                       'warehouse_id':record.source_warehouse_id.id,
                                       'pricelist_id':self.pricelist_id.id,
                                       'company_id':record.source_company_id.id})
            order_vals.sudo().onchange_partner_id()
            order_vals.sudo().onchange_partner_shipping_id()
            if record.crm_team_id:
                order_vals.team_id = record.crm_team_id.id

            sale_order = sale_obj.sudo().create(order_vals._convert_to_write(order_vals._cache))

            # Creates sale order lines.
            for line in record.inter_company_transfer_line_ids:
                line_vals = sale_order_line_obj.sudo().new({'order_id':sale_order.id,
                                                     'product_id':line.product_id})

                # Assigns custom values and calls necessary on_change methods.
                line_vals.sudo().product_id_change()
                line_vals.sudo().update({'product_uom_qty':line.quantity, 'price_unit':line.price})

                sale_order_line_obj.sudo().create(line_vals._convert_to_write(line_vals._cache))

            sale_order.sudo().write({'inter_company_transfer_id':record.id})
    ...
    def create_purchase_order(self):
        ...
        for record in self:
            ...
            order_vals = purchase_obj.sudo().new({'currency_id':record.currency_id.id,
            ...
            purchase_order = purchase_obj.sudo().create(order_vals._convert_to_write(order_vals._cache))

            # Creates purchase order lines.
            for line in record.inter_company_transfer_line_ids:
                line_vals = purchase_line_obj.sudo().new({'order_id':purchase_order.id,
                                                   'product_id':line.product_id,
                                                   'currency_id':self.currency_id})

                # Assigns custom values and calls necessary on_change methods.
                line_vals.sudo().onchange_product_id()
                line_vals.sudo().update({'product_qty':line.quantity,
    ...
    def confirm_orders(self):
        ...
        for record in self:
            ...
            sale_orders.sudo().write({'origin':record.name or ''})
            sale_orders.sudo().with_context(default_type=sale_orders.mapped('warehouse_id.out_type_id.id'),force_company=sale_orders.company_id.id).action_confirm()

            purchase_orders = record.purchase_order_ids.filtered(lambda x:x.state in ['draft', 'sent'])
            purchase_orders.sudo().write({'origin':record.name or ''})
            purchase_orders.sudo().with_context(default_type=purchase_orders.mapped('picking_type_id.id'),force_company=purchase_orders.company_id.id).button_confirm()
                ...
                if inter_company_transfer_ids:
                    ...
                    if delivered_qty > 0.0:
                        created_reverse_line_ids.append(
                            reverse_ict_line_obj.sudo().create({
