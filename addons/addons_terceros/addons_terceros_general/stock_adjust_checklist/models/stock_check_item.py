...
class stock_check_item(models.Model):
    """
    The model to keep each item of a check list
    """
    _name = "stock.check.item"
    _description = "stock Order Check Item"

    @api.model
    def state_selection(self):
        """
        The method to return possible states according to a stock order
        """
        states = self.env["stock.inventory"]._fields["state"]._description_selection(self.env)
        return states

    @api.depends("color_sel")
    def _compute_color_stock(self):
        """
    ...
    group_ids = fields.Many2many(
        "res.groups",
        "res_groups_stock_check_item_list_rel_table",
        "res_groups_id",
        "stock_check_item_id",
    ...
    check_company_list_stock_id = fields.Many2one(
        "check.company.list.stock",
    ...
    def _check_cheklist_rights(self):
        ...
        """
        if not self.env.user.has_group("stock_checklist.group_stock_checklist_superuser"):
            for item in self:
                if item.group_ids:
