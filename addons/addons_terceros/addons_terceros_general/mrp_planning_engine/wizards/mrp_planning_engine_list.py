...
class MRPPlanningEngineList(models.TransientModel):
    ...
    def single_planning_engine_run(self):
        self.message = self.env['mrp.planning.engine.single.run'].planning_engine_single_run(self.mrp_parameter_id)
        self.env['mrp.planning.engine.list.item'].search([]).unlink()
        return self.action_planning_engine_list()
    ...
    def action_planning_engine_list(self):
            ...
            if other_po_line_mrp_parameter and other_po_line_mrp_parameter.supply_method == 'subcontracting':
                ...
                for bomline in bom.bom_line_ids:
                    if bomline.product_id == list.mrp_parameter_id.product_id and not bomline.product_qty <= 0.00:
                        sub_rfq_item_data = list.mrp_parameter_id._prepare_mrp_element_data_from_sub_rfq(other_po_line, bomline)
        ...
        if in_moves:
            for move in in_moves:
                ...
                # exception messages
                # MOs
                sub_locations = self.env['stock.location'].search([('id', 'child_of', move.production_id.company_id.subcontracting_location_id.id), ('company_id', '=', move.production_id.company_id.id)])
                if move.production_id and not (move.production_id.location_dest_id.id in sub_locations.ids):
                    # late MOs
                    if move.production_id.date_planned_start_pivot and move.production_id.date_planned_start_pivot < today_datetime and move.production_id.state in ('draft', 'confirmed'):
