...
class AccountPayment(models.Model):
    ...
    def onchange_date_journal(self):
        domain = [("date", "=", self.date), ("journal_id", "=", self.journal_id.id),('state','=','open')]
    ...
    def add_statement_line(self):
        ...
        for payment in self:
            ...
            if auto_statement and not payment.statement_id and not payment.reconciled_statement_ids:
                domain = [("date", "=", payment.date), ("journal_id", "=", payment.journal_id.id),('state','=','open')]
                statement = self.env["account.bank.statement"].search(domain, limit=1)
                if not statement:
            ...
            if payment.state == "posted" and not payment.statement_line_id and payment.statement_id:
                ...
                payment.statement_id.balance_end_real = payment.statement_id.balance_start + values["amount"]
                payment.statement_id.button_post()
                payment.statement_id.action_bank_reconcile_bank_statements()
                if payment.partner_id:
                    bank_stmt_lines = payment.statement_id.mapped('line_ids')
                    self.env['account.reconciliation.widget'].get_bank_statement_line_data(bank_stmt_lines.ids)
                if len(payment.statement_id.line_ids.filtered(lambda x: not x.is_reconciled)) == 0:
                    payment.statement_id.button_validate_or_action()
